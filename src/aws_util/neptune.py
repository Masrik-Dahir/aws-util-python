"""aws_util.neptune -- Amazon Neptune DB cluster and instance utilities.

Create, describe, modify, delete, snapshot, failover, and poll Neptune
DB clusters and instances.

Boto3 docs: https://docs.aws.amazon.com/boto3/latest/reference/services/neptune.html
"""
...
class NeptuneCluster(BaseModel):
...
class NeptuneInstance(BaseModel):
...
class NeptuneClusterSnapshot(BaseModel):
...
def _parse_cluster(data: dict[str, Any]) -> NeptuneCluster:
    """Parse a Neptune cluster dict into a model."""
    known = {
        "DBClusterIdentifier",
        "Status",
        "Engine",
...
def _parse_instance(data: dict[str, Any]) -> NeptuneInstance:
    """Parse a Neptune instance dict into a model."""
    endpoint = data.get("Endpoint", {})
    known = {
    ...
    return NeptuneInstance(
        ...
        endpoint_address=endpoint.get("Address") if isinstance(endpoint, dict) else None,
...
def _parse_cluster_snapshot(data: dict[str, Any]) -> NeptuneClusterSnapshot:
    """Parse a Neptune cluster snapshot dict into a model."""
    known = {
    ...
    return NeptuneClusterSnapshot(
        db_cluster_snapshot_identifier=data["DBClusterSnapshotIdentifier"],
        db_cluster_identifier=data["DBClusterIdentifier"],
        status=data["Status"],
        engine=data["Engine"],
        engine_version=data.get("EngineVersion", ""),
        snapshot_type=data.get("SnapshotType"),
...
def create_db_cluster(
    ...
    **kwargs: Any,
    ...
    """
    client = get_client("neptune", region_name)
    params: dict[str, Any] = {
    ...
    try:
        resp = client.create_db_cluster(**params)
...
def modify_db_cluster(
    ...
    **kwargs: Any,
    ...
    client = get_client("neptune", region_name)
    try:
        resp = client.modify_db_cluster(DBClusterIdentifier=db_cluster_identifier, **kwargs)
    except ClientError as exc:
...
def modify_db_instance(
    ...
    **kwargs: Any,
    ...
    client = get_client("neptune", region_name)
    try:
        resp = client.modify_db_instance(DBInstanceIdentifier=db_instance_identifier, **kwargs)
    except ClientError as exc:
...
def delete_db_instance(
    ...
    **kwargs: Any,
    ...
    client = get_client("neptune", region_name)
    try:
        client.delete_db_instance(DBInstanceIdentifier=db_instance_identifier, **kwargs)
    except ClientError as exc:
...
def failover_db_cluster(
    ...
    *,
    ...
    try:
        resp = client.failover_db_cluster(**params)
    except ClientError as exc:
        raise wrap_aws_error(exc, "failover_db_cluster failed") from exc
    return _parse_cluster(resp["DBCluster"])


def wait_for_db_cluster(
    db_cluster_identifier: str,
    ...
    *,
    ...
    while True:
        clusters = describe_db_clusters(db_cluster_identifier, region_name=region_name)
        if not clusters:

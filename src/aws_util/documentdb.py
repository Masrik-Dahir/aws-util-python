"""aws_util.documentdb — Amazon DocumentDB (docdb) utilities.

Create, describe, modify, and delete DocumentDB clusters, instances,
and cluster snapshots.  Includes polling helpers (``wait_for_*``) and
...
class ClusterSnapshotResult(BaseModel):
...
def _parse_cluster(data: dict[str, Any]) -> ClusterResult:
    ...
    return ClusterResult(
        ...
        db_subnet_group_name=data.get("DBSubnetGroup"),
...
def _parse_instance(data: dict[str, Any]) -> InstanceResult:
    """Build an :class:`InstanceResult` from a raw API dict."""
    ep = data.get("Endpoint") or {}
    return InstanceResult(
        db_instance_identifier=data["DBInstanceIdentifier"],
        db_instance_class=data.get("DBInstanceClass", "unknown"),
        status=data.get("DBInstanceStatus", "unknown"),
        engine=data.get("Engine", "unknown"),
        engine_version=data.get("EngineVersion", "unknown"),
        endpoint_address=ep.get("Address") if isinstance(ep, dict) else None,
        endpoint_port=ep.get("Port") if isinstance(ep, dict) else None,
...
def _parse_snapshot(data: dict[str, Any]) -> ClusterSnapshotResult:
    """Build a :class:`ClusterSnapshotResult` from a raw API dict."""
...
def create_db_cluster(
    ...
    *,
    ...
    """
    client = get_client("docdb", region_name)
    kwargs: dict[str, Any] = {
...
def describe_db_cluster_snapshots(
    *,
    ...
    try:
        ...
        for page in paginator.paginate(**kwargs):
            for item in page.get("DBClusterSnapshots", []):

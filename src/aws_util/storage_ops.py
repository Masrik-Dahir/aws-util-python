"""storage_ops — Multi-service storage orchestration utilities.

Provides high-level helpers spanning AWS storage services:

- **efs_to_s3_sync** — Create a DataSync task from EFS to S3, start the task
  execution, and track sync state in DynamoDB.
- **fsx_backup_to_s3** — Create an FSx backup, poll until available, and
  record backup metadata in S3.
- **transfer_family_event_processor** — List Transfer Family server executions,
  validate and move files in S3, and notify via SNS.
- **storage_gateway_cache_monitor** — Describe Storage Gateway cache info,
  check utilization against a threshold, and publish a CloudWatch alarm.
- **lightsail_snapshot_to_s3** — Create a Lightsail instance snapshot, export
  to EC2, and record export metadata in S3.
"""
...
class EfsS3SyncResult(BaseModel):
    """Result of :func:`efs_to_s3_sync`."""
...
class FsxBackupResult(BaseModel):
    """Result of :func:`fsx_backup_to_s3`."""
...
class TransferEventResult(BaseModel):
    """Result of :func:`transfer_family_event_processor`."""
...
class CacheMonitorResult(BaseModel):
    """Result of :func:`storage_gateway_cache_monitor`."""

    model_config = ConfigDict(frozen=True)

    gateway_id: str
    cache_used_percent: float
    cache_allocated_bytes: int
    alarm_created: bool


class LightsailExportResult(BaseModel):
    """Result of :func:`lightsail_snapshot_to_s3`."""
...
def efs_to_s3_sync(
    ...
    Creates a DataSync EFS location and S3 location, creates a task linking
    them, starts a task execution, and records the task/execution ARNs in
    DynamoDB for audit tracking.

    Args:
        efs_filesystem_id: EFS file system ID (e.g. ``"fs-12345678"``).
        ...
        key_prefix: Key prefix within the S3 bucket (used as subdirectory).
    ...
    """
    datasync = get_client("datasync", region_name)
    ...
    try:
        ...
        src_resp = datasync.create_location_efs(
            EfsFilesystemArn=(
    ...
    try:
        dst_resp = datasync.create_location_s3(
    ...
    try:
        task_resp = datasync.create_task(
...
def fsx_backup_to_s3(
    ...
    Creates an FSx backup tagged with *backup_name*, polls
    ``describe_backups`` until the backup lifecycle is ``AVAILABLE`` (or
    until a max of 60 iterations at 15-second intervals), then writes a
    JSON metadata object to S3 at *key_prefix*.

    Args:
        file_system_id: FSx file system ID (e.g. ``"fs-12345678"``).
    ...
    Raises:
        RuntimeError: If any underlying AWS API call fails.
    """
    fsx = get_client("fsx", region_name)
    ...
    while backup_status not in terminal_statuses and iteration < max_iterations:
        ...
        iteration += 1
        try:
            desc_resp = fsx.describe_backups(BackupIds=[backup_id])
        except ClientError as exc:
...
def transfer_family_event_processor(
    server_id: str,
    bucket: str,
    ...
    Lists workflow executions for the Transfer Family server, checks that
    each completed-execution file exists in S3, copies it to
    *destination_prefix*, deletes the original, and optionally publishes
    an SNS notification for each moved file.
    ...
    Raises:
        RuntimeError: If any underlying AWS API call fails.
    """
    transfer = get_client("transfer", region_name)
    s3 = get_client("s3", region_name)
    ...
    try:
        exec_resp = transfer.list_executions(WorkflowId=server_id)
    except ClientError as exc:
        # list_executions requires a WorkflowId not a ServerId;
        # fall back to listing the server's users and their home dirs.
        logger.warning(
            "list_executions failed for server %r, falling back to list_users: %s",
            server_id,
            exc,
        )
        exec_resp = {"Executions": []}

    executions: list[dict[str, Any]] = exec_resp.get("Executions", [])
    logger.info("Found %d executions", len(executions))

    # ------------------------------------------------------------------
    # 2. For each completed execution, validate S3 object exists,
    #    copy to destination_prefix, delete original, publish SNS
    # ------------------------------------------------------------------
    for execution in executions:
        ...
        # Extract the source file path from the execution details
        ...
        if sns_topic_arn and sns is not None:
            message = json.dumps(
                {
                    "server_id": server_id,
            ...
            try:
                ...
                raise wrap_aws_error(
                    exc, f"Failed to publish SNS notification to {sns_topic_arn!r}"
                ) from exc
...
def storage_gateway_cache_monitor(
    gateway_arn: str,
    ...
    Calls ``describe_cache`` to retrieve the gateway's cache utilization.
    If utilization exceeds *alarm_threshold_percent*, calls
    ``put_metric_alarm`` on CloudWatch.
    ...
    Raises:
        RuntimeError: If any underlying AWS API call fails.
    """
    sgw = get_client("storagegateway", region_name)
    ...
    try:
        cache_resp = sgw.describe_cache(GatewayARN=gateway_arn)
    except ClientError as exc:
...
def lightsail_snapshot_to_s3(
    ...
    Creates a Lightsail snapshot of *instance_name*, polls
    ``get_instance_snapshot`` until the snapshot state is ``available``,
    calls ``export_snapshot`` to initiate an EC2 export, then writes a
    JSON metadata object to S3.
    ...
    Raises:
        RuntimeError: If any underlying AWS API call fails.
    """
    ls = get_client("lightsail", region_name)
    s3 = get_client("s3", region_name)
    ...
    while snapshot_state != "available" and iteration < max_iterations:
        ...
        iteration += 1
        try:
            snap_resp = ls.get_instance_snapshot(instanceSnapshotName=snapshot_name)
        except ClientError as exc:
            raise wrap_aws_error(
    ...
    if snapshot_state == "available":
        ...
        try:
            export_resp = ls.export_snapshot(sourceSnapshotName=snapshot_name)

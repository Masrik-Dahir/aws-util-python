...
def run_log_insights_query(
    ...
    for result_row in result_resp.get("results", []):
        ...
        for field in result_row:
            row[field["field"]] = str(field["value"])
        rows.append(row)
...
# ---------------------------------------------------------------------------
# Models — new observability additions
# ---------------------------------------------------------------------------


class KinesisAlarmResult(BaseModel):
    """Result of :func:`kinesis_analytics_alarm_manager`."""
...
class DMSMonitorResult(BaseModel):
    """Result of :func:`dms_task_monitor`."""
...
class HealthEventResult(BaseModel):
    """Result of :func:`health_event_to_teams`."""
...
class QuotaMonitorResult(BaseModel):
    """Result of :func:`service_quota_monitor`."""
...
def kinesis_analytics_alarm_manager(
    application_name: str,
    sns_topic_arn: str,
    alarm_prefix: str,
    milliseconds_behind_threshold: int = 60000,
    region_name: str | None = None,
) -> KinesisAlarmResult:
    """Create CloudWatch alarms for a Kinesis Data Analytics application.

    Verifies the application exists, then creates alarms for
    ``MillisBehindLatest`` and ``Records`` metrics wired to SNS.

    Args:
        application_name: Kinesis Data Analytics application name.
        sns_topic_arn: SNS topic ARN for alarm notifications.
        alarm_prefix: Prefix for CloudWatch alarm names.
        milliseconds_behind_threshold: Threshold for MillisBehindLatest in ms
            (default ``60000``).
        region_name: AWS region override.

    Returns:
        A :class:`KinesisAlarmResult` with application status and alarm names.

    Raises:
        RuntimeError: If any AWS API call fails.
    """
    kda = get_client("kinesisanalyticsv2", region_name)
    cw = get_client("cloudwatch", region_name)

    try:
        app_resp = kda.describe_application(ApplicationName=application_name)
    except ClientError as exc:
        raise wrap_aws_error(
            exc,
            f"kinesis_analytics_alarm_manager describe_application failed for {application_name!r}",
        ) from exc

    app_status: str = app_resp.get("ApplicationDetail", {}).get("ApplicationStatus", "UNKNOWN")
    dims = [{"Name": "Application", "Value": application_name}]
    alarm_specs: list[dict[str, Any]] = [
        {
            "suffix": "MillisBehindLatest",
            "metric": "millisBehindLatest",
            "namespace": "AWS/KinesisAnalytics",
            "stat": "Maximum",
            "threshold": float(milliseconds_behind_threshold),
            "comparison": "GreaterThanOrEqualToThreshold",
        },
        {
            "suffix": "Records",
            "metric": "numRecordsInPerSecond",
            "namespace": "AWS/KinesisAnalytics",
            "stat": "Average",
            "threshold": 0.0,
    ...
    for spec in alarm_specs:
        alarm_name = f"{alarm_prefix}-{application_name}-{spec['suffix']}"
        try:
            cw.put_metric_alarm(
                AlarmName=alarm_name,
            ...
            raise wrap_aws_error(
                exc, f"kinesis_analytics_alarm_manager put_metric_alarm failed for {alarm_name!r}"
            ) from exc

    return KinesisAlarmResult(
        application_status=app_status,
        alarms_created=len(alarm_names),
...
def dms_task_monitor(
    replication_task_arn: str,
    metric_namespace: str,
    region_name: str | None = None,
) -> DMSMonitorResult:
    """Monitor a DMS replication task and emit per-table row metrics to CloudWatch.

    Describes the task, fetches table statistics, and emits Inserts/Deletes/Updates
    per table as CloudWatch custom metrics.
    ...
    Returns:
        A :class:`DMSMonitorResult` with task status and metric counts.

    Raises:
        RuntimeError: If any AWS API call fails.
    """
    dms = get_client("dms", region_name)
    cw = get_client("cloudwatch", region_name)

    try:
        task_resp = dms.describe_replication_tasks(
            Filters=[{"Name": "replication-task-arn", "Values": [replication_task_arn]}]
        )
    except ClientError as exc:
        raise wrap_aws_error(exc, "dms_task_monitor describe_replication_tasks failed") from exc

    tasks = task_resp.get("ReplicationTasks", [])
    task_status = tasks[0].get("Status", "unknown") if tasks else "unknown"

    try:
        stats_resp = dms.describe_table_statistics(ReplicationTaskArn=replication_task_arn)
    except ClientError as exc:
        raise wrap_aws_error(exc, "dms_task_monitor describe_table_statistics failed") from exc

    table_stats = stats_resp.get("TableStatistics", [])
    metrics_published = 0
    total_rows = 0

    for table in table_stats:
        schema = table.get("SchemaName", "unknown")
        tbl_name = table.get("TableName", "unknown")
        dims = [
            {"Name": "ReplicationTaskArn", "Value": replication_task_arn},
            {"Name": "Table", "Value": f"{schema}.{tbl_name}"},
        ]
        inserts = table.get("Inserts", 0)
        deletes = table.get("Deletes", 0)
        updates = table.get("Updates", 0)
        total_rows += inserts + deletes + updates

        metric_data = [
            {"MetricName": "Inserts", "Value": float(inserts), "Unit": "Count", "Dimensions": dims},
            {"MetricName": "Deletes", "Value": float(deletes), "Unit": "Count", "Dimensions": dims},
            {"MetricName": "Updates", "Value": float(updates), "Unit": "Count", "Dimensions": dims},
        ]
        try:
            ...
            raise wrap_aws_error(
                exc, f"dms_task_monitor put_metric_data failed for {schema}.{tbl_name}"
            ) from exc

    return DMSMonitorResult(
...
def health_event_to_teams(
    queue_url: str,
    lookback_hours: int = 24,
    region_name: str | None = None,
) -> HealthEventResult:
    """Fetch recent AWS Health events and enqueue enriched messages to SQS.

    Queries AWS Health for recent events, enriches them with account names
    from AWS Organizations, and sends messages to SQS for downstream webhook
    delivery.

    Args:
        queue_url: SQS queue URL for enriched event messages.
        lookback_hours: Hours of history to scan (default ``24``).
        region_name: AWS region override.

    Returns:
        A :class:`HealthEventResult` with event and message counts.

    Raises:
        RuntimeError: If any AWS API call fails.
    """
    health = get_client("health", "us-east-1")  # Health API is global, must use us-east-1
    orgs = get_client("organizations", region_name)
    sqs = get_client("sqs", region_name)

    start_ts = time.time() - lookback_hours * 3600
    start_dt = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime(start_ts))

    try:
        events_resp = health.describe_events(
            filter={"startTimes": [{"from": start_dt}]},
            maxResults=100,
        )
    except ClientError as exc:
        raise wrap_aws_error(exc, "health_event_to_teams describe_events failed") from exc

    events_list = events_resp.get("events", [])
    if not events_list:
        return HealthEventResult(events_found=0, messages_sent=0, affected_accounts=[])

    # Fetch affected accounts for all events
    event_arns = [e["arn"] for e in events_list if "arn" in e]
    affected_account_ids: list[str] = []
    if event_arns:
        try:
            affected_resp = health.describe_affected_accounts(eventArn=event_arns[0])
            affected_account_ids = affected_resp.get("affectedAccounts", [])
        except ClientError:
            pass

    # Enrich with org account names
    account_names: dict[str, str] = {}
    try:
        paginator = orgs.get_paginator("list_accounts")
        for page in paginator.paginate():
            for acct in page.get("Accounts", []):
                account_names[acct["Id"]] = acct.get("Name", acct["Id"])
    except ClientError:
        pass
    ...
    for event in events_list:
        enriched = {
            ...
            "affectedAccounts": [
                {"id": aid, "name": account_names.get(aid, aid)} for aid in affected_account_ids
            ],
        }
        try:
            sqs.send_message(QueueUrl=queue_url, MessageBody=json.dumps(enriched, default=str))
            messages_sent += 1
        except ClientError as exc:
            raise wrap_aws_error(exc, "health_event_to_teams send_message failed") from exc

    return HealthEventResult(
        events_found=len(events_list),
...
def service_quota_monitor(
    service_codes: list[str],
    table_name: str,
    alarm_prefix: str,
    region_name: str | None = None,
) -> QuotaMonitorResult:
    """Monitor Service Quotas and create CloudWatch alarms near the limit.

    For each service code, retrieves current quotas, checks DynamoDB for
    custom thresholds, and creates CloudWatch alarms when utilization
    approaches the limit.

    Args:
        service_codes: List of AWS service codes (e.g. ``["ec2", "lambda"]``).
        table_name: DynamoDB table name containing custom thresholds.
            Items must have ``pk`` (service_code#quota_code) and
            ``threshold_pct`` (number 0-100).
        alarm_prefix: Prefix for CloudWatch alarm names.
        region_name: AWS region override.

    Returns:
        A :class:`QuotaMonitorResult` with check and alarm counts.

    Raises:
        RuntimeError: If any AWS API call fails.
    """
    sq = get_client("service-quotas", region_name)
    ...
    for service_code in service_codes:
        ...
        try:
            ...
            raise wrap_aws_error(
                exc, f"service_quota_monitor list_service_quotas failed for {service_code!r}"
            ) from exc

        for quota in quotas:
            quota_code = quota.get("QuotaCode", "")
            quota_value = float(quota.get("Value", 0) or 0)
            if quota_value == 0:
                continue

            # Check DynamoDB for custom threshold
            threshold_pct = 80.0
            pk = f"{service_code}#{quota_code}"
            try:
                ddb_resp = dynamo.get_item(
                    TableName=table_name,
                    Key={"pk": {"S": pk}},
                )
                item = ddb_resp.get("Item", {})
                if item.get("threshold_pct", {}).get("N"):
                    threshold_pct = float(item["threshold_pct"]["N"])
            except ClientError:
                pass

            threshold_value = quota_value * threshold_pct / 100.0
            alarm_name = f"{alarm_prefix}-{service_code}-{quota_code}"
            ...
            try:
                cw.put_metric_alarm(
                    ...
                    Dimensions=[
                        {"Name": "ServiceCode", "Value": service_code},
                ...
                raise wrap_aws_error(
                    exc, f"service_quota_monitor put_metric_alarm failed for {alarm_name!r}"
                ) from exc

    return QuotaMonitorResult(
        services_checked=services_checked,
        quotas_near_limit=quotas_near_limit,

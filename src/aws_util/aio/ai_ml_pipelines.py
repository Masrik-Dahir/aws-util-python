...
async def rekognition_face_indexer(
    collection_id: str,
    bucket: str,
    keys: list[str],
    table_name: str,
    ...
    For each image key, calls Rekognition ``IndexFaces`` and stores the
    ...
    Raises:
        RuntimeError: If any Rekognition or DynamoDB call fails.
    """
    rek = async_client("rekognition", region_name)
    ddb = async_client("dynamodb", region_name)
    ...
    try:
        await rek.call("CreateCollection", CollectionId=collection_id)
    ...
    for key, user_id in zip(keys, user_ids, strict=False):
        try:
            ...
            raise wrap_aws_error(exc, f"Rekognition IndexFaces failed for {key}") from exc
...
async def rekognition_video_label_pipeline(
    bucket: str,
    video_key: str,
    table_name: str,
    sns_topic_arn: str,
    ...
    video object, polls ``GetLabelDetection`` until ``SUCCEEDED`` or
    ``FAILED``, then writes all labels to DynamoDB.
    ...
    Raises:
        RuntimeError: If Rekognition or DynamoDB calls fail, or job fails.
    """
    rek = async_client("rekognition", region_name)
    ddb = async_client("dynamodb", region_name)

    try:
        start_resp = await rek.call(
            "StartLabelDetection",
    ...
    while status == "IN_PROGRESS":
        ...
        try:
            poll_resp = await rek.call("GetLabelDetection", JobId=job_id, SortBy="TIMESTAMP")
        except RuntimeError as exc:
...
async def polly_audio_generator(
    text: str,
    bucket: str,
    voice_id: str = "Joanna",
    ...
    Calls Polly ``SynthesizeSpeech``, writes the audio stream to S3
    ...
    """
    polly = async_client("polly", region_name)
    s3 = async_client("s3", region_name)
    ...
    try:
        ...
        raise wrap_aws_error(exc, "Polly SynthesizeSpeech failed") from exc

    audio_raw = polly_resp.get("AudioStream", b"")
    if hasattr(audio_raw, "read"):
        audio_stream = audio_raw.read()
    elif isinstance(audio_raw, (bytes, bytearray)):
        audio_stream = audio_raw
    else:
        audio_stream = b""
    ...
    try:
        presigned_resp = await s3.call(
            "GeneratePresignedUrl",
            ClientMethod="get_object",
            Params={"Bucket": bucket, "Key": s3_key},
            ExpiresIn=presign_expiry,
        )
        presigned_url = presigned_resp if isinstance(presigned_resp, str) else str(presigned_resp)
    except RuntimeError as exc:
...
async def transcribe_job_to_s3(
    job_name: str,
    bucket: str,
    audio_key: str,
    output_bucket: str,
    ...
    ``GetTranscriptionJob`` until ``COMPLETED`` or ``FAILED``, reads the
    transcript JSON from the Transcribe-provided URI, then copies the
    transcript to *output_bucket* under *output_prefix*.
    ...
    while status in ("IN_PROGRESS", "QUEUED"):
        ...
        try:
            poll_resp = await transcribe.call("GetTranscriptionJob", TranscriptionJobName=job_name)
        except RuntimeError as exc:
...
async def comprehend_pii_redactor(
    text: str,
    bucket: str,
    output_key: str,
    ...
    Calls Comprehend ``DetectPiiEntities`` on the input text, replaces
    each detected entity span with *replacement* (processed in reverse
    offset order to preserve accuracy), then uploads the sanitized text
    to S3 at *output_key*.
    ...
    try:
        pii_resp = await comprehend.call(
            "DetectPiiEntities",
            Text=text,
            LanguageCode=language_code,
        )
    except RuntimeError as exc:
        raise wrap_aws_error(exc, "Comprehend DetectPiiEntities failed") from exc
...
async def textract_form_extractor(
    bucket: str,
    key: str,
    table_name: str,
    region_name: str | None = None,
    ...
    Calls Textract ``AnalyzeDocument`` with ``FeatureTypes=["FORMS"]``,
    ...
    try:
        resp = await textract.call(
            "AnalyzeDocument",
            Document={"S3Object": {"Bucket": bucket, "Name": key}},
            FeatureTypes=["FORMS"],
        )
    except RuntimeError as exc:
        raise wrap_aws_error(exc, f"Textract AnalyzeDocument failed for {key}") from exc
    ...
    def get_text_for_block(block: dict) -> str:
        text_parts: list[str] = []
...
async def bedrock_knowledge_base_ingestor(
    ...
    ``GetIngestionJob`` until ``COMPLETE`` or ``FAILED``.
    ...
    """
    s3 = async_client("s3", region_name)
    bedrock_agent = async_client("bedrock-agent", region_name)

    for doc_key in document_keys:
        try:
            await s3.call("HeadObject", Bucket=bucket, Key=doc_key)
        except RuntimeError as exc:
            raise wrap_aws_error(exc, f"S3 object s3://{bucket}/{doc_key} not found") from exc

    try:
        start_resp = await bedrock_agent.call(
            "StartIngestionJob",
    ...
    while status in ("STARTING", "IN_PROGRESS"):
        await asyncio.sleep(10)
        try:
            poll_resp = await bedrock_agent.call(
                "GetIngestionJob",
                knowledgeBaseId=knowledge_base_id,
                dataSourceId=data_source_id,
...
async def bedrock_guardrail_enforcer(
    guardrail_id: str,
    ...
    Calls Bedrock Runtime ``ApplyGuardrail`` for the given text and
    ...
    """
    bedrock_runtime = async_client("bedrock-runtime", region_name)
    logs = async_client("logs", region_name)

    try:
        guardrail_resp = await bedrock_runtime.call(
            "ApplyGuardrail",
...
async def personalize_real_time_recommender(
    tracking_id: str,
    campaign_arn: str,
    user_id: str,
    item_id: str,
    ...
    ``PutEvents``, then calls ``GetRecommendations`` on the given
    ...
    """
    personalize_events = async_client("personalize-events", region_name)
    personalize_runtime = async_client("personalize-runtime", region_name)

    event_recorded = False
    try:
        await personalize_events.call(
            "PutEvents",
    ...
    try:
        rec_resp = await personalize_runtime.call(
            "GetRecommendations",
...
async def forecast_inference_pipeline(
    forecast_arn: str,
    s3_path: str,
    iam_role_arn: str,
    table_name: str,
    ...
    ``DescribeForecastExportJob`` until ``ACTIVE`` or ``CREATE_FAILED``,
    reads resulting CSV(s) from S3, and writes each prediction row into
    DynamoDB.
    ...
    forecast = async_client("forecast", region_name)
    s3 = async_client("s3", region_name)
    ddb = async_client("dynamodb", region_name)
    ...
    try:
        export_resp = await forecast.call(
    ...
    ts = int(time.time())
    try:
        list_resp = await s3.call("ListObjectsV2", Bucket=export_bucket, Prefix=export_prefix)
    except RuntimeError as exc:
    ...
    for obj in list_resp.get("Contents", []):
        ...
        try:
            csv_resp = await s3.call("GetObject", Bucket=export_bucket, Key=obj_key)
            body_raw = csv_resp.get("Body", b"")
            if isinstance(body_raw, (bytes, bytearray)):
                csv_content = body_raw.decode("utf-8")
            elif hasattr(body_raw, "read"):
                csv_content = body_raw.read().decode("utf-8")
            else:
                csv_content = str(body_raw)
        except RuntimeError as exc:
...
async def sagemaker_batch_transform_monitor(
    job_name: str,
    model_name: str,
    input_s3_uri: str,
    output_s3_uri: str,
    ...
    locations, polls ``DescribeTransformJob`` until ``Completed`` or
    ``Failed``, then sends the output S3 URI to the specified SQS queue.
    ...
    while status in ("InProgress", "Stopping"):
        await asyncio.sleep(15)
        try:
            desc_resp = await sm.call("DescribeTransformJob", TransformJobName=job_name)
        except RuntimeError as exc:

"""Native async sagemaker_runtime utilities using :mod:`aws_util.aio._engine`."""
...
async def invoke_endpoint(
    ...
    """
    client = async_client("sagemaker-runtime", region_name)
    kwargs: dict[str, Any] = {
    ...
    try:
        resp = await client.call("InvokeEndpoint", **kwargs)
    except RuntimeError:
        raise
    ...
    elif hasattr(resp_body, "read"):
        raw = resp_body.read()
        resp_body = raw.decode("utf-8") if isinstance(raw, bytes) else raw
...
async def invoke_endpoint_async(
    ...
    if custom_attributes is not None:
        kwargs["CustomAttributes"] = custom_attributes
    try:
        resp = await client.call("InvokeEndpointAsync", **kwargs)
    except RuntimeError:
        raise
    except Exception as exc:
...
async def invoke_endpoint_with_response_stream(
    ...
    if custom_attributes is not None:
        kwargs["CustomAttributes"] = custom_attributes
    try:
        resp = await client.call("InvokeEndpointWithResponseStream", **kwargs)
    except RuntimeError:
        raise
    except Exception as exc:

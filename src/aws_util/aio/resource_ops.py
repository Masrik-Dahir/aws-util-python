...
# ---------------------------------------------------------------------------
# 11. SES suppression list manager (async)
# ---------------------------------------------------------------------------


async def ses_suppression_list_manager(
    bucket: str,
    blocklist_key: str,
    table_name: str,
    region_name: str | None = None,
) -> SuppressionSyncResult:
    """Sync the SES account suppression list with a canonical S3 blocklist (async).

    Reads the current SES suppressed destinations, downloads the authoritative
    blocklist from S3, computes the diff, adds new entries to SES, removes
    stale entries, and records operations in DynamoDB for audit.

    Args:
        bucket: S3 bucket containing the blocklist.
        blocklist_key: S3 key of the blocklist file (one email per line or JSON array).
        table_name: DynamoDB table for deduplication and audit records.
        region_name: AWS region override.

    Returns:
        A :class:`SuppressionSyncResult` with added/removed counts.
    ...
    # Fetch current SES suppression list
    current_emails: set[str] = set()
    try:
        resp = await sesv2.call("ListSuppressedDestinations")
        for dest in resp.get("SuppressedDestinationSummaries", []):
            current_emails.add(dest["EmailAddress"].lower())
    except RuntimeError as exc:
        raise wrap_aws_error(
            exc, "ses_suppression_list_manager list_suppressed_destinations failed"
        ) from exc

    # Download blocklist from S3
    s3_synced = False
    try:
        obj_resp = await s3.call("GetObject", Bucket=bucket, Key=blocklist_key)
        body = obj_resp["Body"]
        if hasattr(body, "read"):
            raw = body.read().decode("utf-8").strip()
        else:
            raw = body.decode("utf-8").strip() if isinstance(body, bytes) else str(body)
        try:
            blocklist: list[str] = [e.lower() for e in json.loads(raw) if e]
        except (json.JSONDecodeError, TypeError):
            blocklist = [line.strip().lower() for line in raw.splitlines() if line.strip()]
        s3_synced = True
    except RuntimeError as exc:
        raise wrap_aws_error(
            exc,
            f"ses_suppression_list_manager S3 download failed for s3://{bucket}/{blocklist_key}",
        ) from exc

    blocklist_set = set(blocklist)
    to_add = blocklist_set - current_emails
    to_remove = current_emails - blocklist_set

    added = 0
    for email in to_add:
        try:
            await sesv2.call("PutSuppressedDestination", EmailAddress=email, Reason="COMPLAINT")
            await dynamo.call(
                "PutItem",
                TableName=table_name,
                Item={
            ...
            raise wrap_aws_error(
                exc, f"ses_suppression_list_manager add failed for {email!r}"
            ) from exc

    removed = 0
    for email in to_remove:
        try:
            await sesv2.call("DeleteSuppressedDestination", EmailAddress=email)
            await dynamo.call(
                "PutItem",
                TableName=table_name,
                Item={
                    "pk": {"S": email},
                    "status": {"S": "removed"},
                    "source": {"S": "blocklist-sync"},
                },
            )
            removed += 1
        except RuntimeError as exc:
            raise wrap_aws_error(
                exc, f"ses_suppression_list_manager remove failed for {email!r}"
            ) from exc

    return SuppressionSyncResult(

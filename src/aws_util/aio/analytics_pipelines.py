"""Native async analytics_pipelines — Analytics Pipeline utilities.
...
async def redshift_unload_to_s3(
    ...
    Args:
        ...
        s3_path: S3 destination path for the UNLOAD output.
    ...
    """
    redshift_data = async_client("redshift-data", region_name)
    ...
    try:
        exec_resp = await redshift_data.call(
            "ExecuteStatement",
    ...
    for _ in range(60):
        try:
            desc = await redshift_data.call("DescribeStatement", Id=statement_id)
        except RuntimeError as exc:
...
async def redshift_serverless_query_runner(
    workgroup_name: str,
    database: str,
    sql: str,
    bucket: str,
    ...
    while True:
        try:
            page = await redshift_data.call("GetStatementResult", **paginator_kwargs)
        except RuntimeError as exc:
...
async def quicksight_dashboard_embedder(
    aws_account_id: str,
    dashboard_id: str,
    ...
    """
    qs = async_client("quicksight", region_name)
...
async def quicksight_dataset_refresher(
    aws_account_id: str,
    dataset_id: str,
    ingestion_id: str,
...
async def athena_result_to_dynamodb(
    query: str,
    database: str,
    output_location: str,
    table_name: str,
    ...
    *key_column* as the partition key.
    ...
    for _ in range(60):
        try:
            exec_resp = await athena.call("GetQueryExecution", QueryExecutionId=execution_id)
        except RuntimeError as exc:
    ...
    while True:
        try:
            page = await athena.call("GetQueryResults", **paginator_kwargs)
        except RuntimeError as exc:
...
async def glue_crawler_and_catalog_sync(
    crawler_name: str,
    table_name: str,
    reference_key: str,
    region_name: str | None = None,
    ...
    try:
        await glue.call("StartCrawler", Name=crawler_name)
    except RuntimeError as exc:
    ...
    for _ in range(120):
        try:
            desc = await glue.call("GetCrawler", Name=crawler_name)
        except RuntimeError as exc:
    ...
    catalog_targets = crawler_info.get("Targets", {}).get("CatalogTargets", [])
    db_name = catalog_targets[0].get("DatabaseName", "") if catalog_targets else ""
    ...
    if db_name:
        try:
            tables_resp = await glue.call("GetTables", DatabaseName=db_name)
...
async def glue_databrew_profile_pipeline(
    job_name: str,
    ...
    """
    databrew = async_client("databrew", region_name)
    cw = async_client("cloudwatch", region_name)

    try:
        run_resp = await databrew.call("StartJobRun", Name=job_name)
    except RuntimeError as exc:
    ...
    for _ in range(120):
        try:
            desc = await databrew.call("DescribeJobRun", Name=job_name, RunId=run_id)
        except RuntimeError as exc:
...
async def emr_serverless_job_runner(
    application_id: str,
    ...
    """
    emr = async_client("emr-serverless", region_name)
    cw = async_client("cloudwatch", region_name)
    ddb = async_client("dynamodb", region_name)

    try:
        start_resp = await emr.call(
            "StartJobRun",
...
async def timestream_query_to_s3(
    query_string: str,
    bucket: str,
    output_key: str,
    region_name: str | None = None,
    ...
    """
    ts_query = async_client("timestream-query", region_name)
    s3 = async_client("s3", region_name)
    ...
    while True:
        try:
            page = await ts_query.call("Query", **paginator_kwargs)
        except RuntimeError as exc:
...
async def neptune_graph_query_to_s3(
    ...
    """
    neptune = async_client("neptune-graph", region_name)
    s3 = async_client("s3", region_name)

    try:
        resp = await neptune.call(
            "ExecuteQuery",
...
async def elbv2_access_log_analyzer(
    ...
    for _ in range(30):
        try:
            qexec = await athena.call("GetQueryExecution", QueryExecutionId=query_execution_id)
        except RuntimeError as exc:
    ...
    if qstate == "SUCCEEDED":
        try:
            results = await athena.call("GetQueryResults", QueryExecutionId=query_execution_id)
...
async def glue_job_output_to_redshift(
    job_name: str,
    ...
    try:
        run_resp = await glue.call("StartJobRun", JobName=job_name, Arguments=job_arguments)
    except RuntimeError as exc:
    ...
    for _ in range(120):
        try:
            desc = await glue.call("GetJobRun", JobName=job_name, RunId=job_run_id)
        except RuntimeError as exc:
    ...
    try:
        copy_resp = await redshift_data.call(
            "ExecuteStatement",
            WorkgroupName=workgroup_name,
            Database=database,
            Sql=copy_sql,
    ...
    for _ in range(60):
        try:
            copy_desc = await redshift_data.call("DescribeStatement", Id=copy_statement_id)
        except RuntimeError as exc:
            raise wrap_aws_error(
...
async def opensearch_index_lifecycle_manager(
    ...
    """
    oss = async_client("opensearchserverless", region_name)
    cw = async_client("cloudwatch", region_name)
    ...
    try:
        resp = await oss.call("BatchGetCollection", names=collection_names)
    except RuntimeError as exc:

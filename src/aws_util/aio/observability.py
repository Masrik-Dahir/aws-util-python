...
# ---------------------------------------------------------------------------
# New observability additions (async)
# ---------------------------------------------------------------------------


async def kinesis_analytics_alarm_manager(
    ...
    """Create CloudWatch alarms for a Kinesis Data Analytics application (async).
    ...
    Returns:
        A :class:`KinesisAlarmResult`.

    Raises:
        RuntimeError: If any AWS API call fails.
    """
    kda = async_client("kinesisanalyticsv2", region_name)
    cw = async_client("cloudwatch", region_name)

    try:
        app_resp = await kda.call("DescribeApplication", ApplicationName=application_name)
    except RuntimeError as exc:
...
async def dms_task_monitor(
    replication_task_arn: str,
    metric_namespace: str,
    region_name: str | None = None,
) -> DMSMonitorResult:
    """Monitor a DMS replication task and emit per-table row metrics (async).
    ...
    Returns:
        A :class:`DMSMonitorResult`.

    Raises:
        RuntimeError: If any AWS API call fails.
    """
    dms = async_client("dms", region_name)
    cw = async_client("cloudwatch", region_name)

    try:
        task_resp = await dms.call(
    ...
    for table in table_stats:
        ...
        try:
            await cw.call("PutMetricData", Namespace=metric_namespace, MetricData=metric_data)
...
async def health_event_to_teams(
    queue_url: str,
    lookback_hours: int = 24,
    region_name: str | None = None,
) -> HealthEventResult:
    """Fetch recent AWS Health events and enqueue enriched messages to SQS (async).
    ...
    health = async_client("health", "us-east-1")
    orgs = async_client("organizations", region_name)
    ...
    if event_arns:
        try:
            affected_resp = await health.call("DescribeAffectedAccounts", eventArn=event_arns[0])
    ...
    try:
        accounts_resp = await orgs.call("ListAccounts")
...
async def service_quota_monitor(
    service_codes: list[str],
    table_name: str,
    alarm_prefix: str,
    region_name: str | None = None,
) -> QuotaMonitorResult:
    """Monitor Service Quotas and create CloudWatch alarms near the limit (async).
    ...
    Returns:
        A :class:`QuotaMonitorResult`.

    Raises:
        RuntimeError: If any AWS API call fails.
    """
    sq = async_client("service-quotas", region_name)
    ...
    for service_code in service_codes:
        services_checked += 1
        try:
            quotas_resp = await sq.call("ListServiceQuotas", ServiceCode=service_code)
            quotas: list[dict[str, Any]] = quotas_resp.get("Quotas", [])
        except RuntimeError as exc:
            raise wrap_aws_error(

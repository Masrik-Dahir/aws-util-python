"""Native async ci_cd_ops utilities using :mod:`aws_util.aio._engine`.

Provides async counterparts for every function in :mod:`aws_util.ci_cd_ops`.
Models are re-exported from the sync module — there is no duplication.

- **codebuild_triggered_deploy** — async CodeBuild + CodeDeploy pipeline.
- **codepipeline_approval_notifier** — async SES v2 approval emails + DynamoDB.
- **codecommit_pr_to_codebuild** — async PR build trigger + PR comment.
- **codeartifact_package_promoter** — async package copy + SSM list update.
"""
...
async def codebuild_triggered_deploy(
    project_name: str,
    source_bucket: str,
    source_key: str,
    ...
    Async counterpart of :func:`aws_util.ci_cd_ops.codebuild_triggered_deploy`.
    ...
    cb = async_client("codebuild", region_name)
    cd = async_client("codedeploy", region_name)
    ...
    while build_status not in terminal_statuses:
        ...
        try:
            batch_resp = await cb.call("BatchGetBuilds", ids=[build_id])
        except RuntimeError as exc:
    ...
    if build_status == "SUCCEEDED":
        ...
        try:
            ...
            dep_info = await cd.call("GetDeployment", deploymentId=deployment_id)
...
async def codepipeline_approval_notifier(
    pipeline_name: str,
    table_name: str,
    from_email: str,
    to_emails: list[str],
    ...
    Async counterpart of :func:`aws_util.ci_cd_ops.codepipeline_approval_notifier`.
    ...
    Raises:
        RuntimeError: If any underlying AWS API call fails.
    """
    cp = async_client("codepipeline", region_name)
    sesv2 = async_client("sesv2", region_name)
    ...
    try:
        state_resp = await cp.call("GetPipelineState", name=pipeline_name)
    except RuntimeError as exc:
    ...
    for stage in stage_states:
        ...
        for action_state in stage.get("actionStates", []):
            ...
            try:
                ...
                raise wrap_aws_error(
                    exc,
                    f"Failed to record approval token in DynamoDB table {table_name!r}",
                ) from exc
...
async def codecommit_pr_to_codebuild(
    repository_name: str,
    pull_request_id: str,
    project_name: str,
    ...
    Async counterpart of :func:`aws_util.ci_cd_ops.codecommit_pr_to_codebuild`.
    ...
    cc = async_client("codecommit", region_name)
    cb = async_client("codebuild", region_name)

    # ------------------------------------------------------------------
    # 1. Start CodeBuild with the PR's source reference
    # ------------------------------------------------------------------
    logger.info(
    ...
    # ------------------------------------------------------------------
    try:
        pr_detail = await cc.call("GetPullRequest", pullRequestId=pull_request_id)
    except RuntimeError as exc:
        raise wrap_aws_error(
...
async def codeartifact_package_promoter(
    source_domain: str,
    ...
    Async counterpart of :func:`aws_util.ci_cd_ops.codeartifact_package_promoter`.
    ...
    Raises:
        RuntimeError: If any underlying AWS API call fails.
    """
    ca = async_client("codeartifact", region_name)
    ...
    try:
        param_resp = await ssm.call("GetParameter", Name=ssm_parameter_name)
        ...
        error_msg = str(exc)
        if "ParameterNotFound" in error_msg:
            package_list = []
        else:
            raise wrap_aws_error(

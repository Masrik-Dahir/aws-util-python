"""Native async Cache Operations — real non-blocking I/O via :mod:`aws_util.aio._engine`.

Async counterpart to :mod:`aws_util.cache_ops`.  Models and helpers are re-exported
...
_MEMORYDB_POLL_INTERVAL = 15.0  # seconds
_MEMORYDB_MAX_WAIT = 900.0  # 15 minutes
...
async def elasticache_warmer(
    ...
    """Describe ElastiCache endpoint and prepare seed data from DynamoDB (async).
    ...
    """
    from aws_util._client import get_client as _get_client

    dynamodb = async_client("dynamodb", region_name)

    # _describe_elasticache_endpoint is sync (boto3) — run in a thread
    ec_sync = _get_client("elasticache", region_name)
    endpoint, port = await asyncio.to_thread(_describe_elasticache_endpoint, ec_sync, cluster_id)
    ...
    while True:
        try:
            scan_resp = await dynamodb.call("Scan", **scan_kwargs)
...
async def elasticache_cache_invalidator(
    cluster_id: str,
    stream_records: list[dict[str, Any]],
    key_prefix: str = "",
    region_name: str | None = None,
) -> CacheInvalidationResult:
    """Extract invalidation keys from DynamoDB stream event records and look up cluster endpoint (async).
    ...
    # Describe ElastiCache endpoint (sync helper — run in thread)
    ec_sync = _get_client("elasticache", region_name)
    endpoint, port = await asyncio.to_thread(_describe_elasticache_endpoint, ec_sync, cluster_id)
...
async def memorydb_snapshot_to_s3(
    cluster_name: str,
    snapshot_name: str,
    bucket: str,
    ...
    """Create a MemoryDB snapshot, wait for completion, copy metadata to S3 (async).
    ...
    """
    memorydb = async_client("memorydb", region_name)
    s3 = async_client("s3", region_name)

    # Create snapshot
    try:
        create_resp = await memorydb.call(
            "CreateSnapshot",
    ...
    deadline = time.monotonic() + _MEMORYDB_MAX_WAIT
    while True:
        try:
            desc_resp = await memorydb.call("DescribeSnapshots", SnapshotName=snapshot_name)
        ...
        if time.monotonic() + _MEMORYDB_POLL_INTERVAL > deadline:
            raise wrap_aws_error(
                TimeoutError(
        ...
        logger.debug(
            "MemoryDB snapshot %s status: %s — waiting %.0fs",
            snapshot_name,
            status,
            _MEMORYDB_POLL_INTERVAL,
        )
        await asyncio.sleep(_MEMORYDB_POLL_INTERVAL)

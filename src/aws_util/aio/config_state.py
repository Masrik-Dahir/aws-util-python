...
async def cross_account_role_assumer(
    ...
    for arn in role_arns:
        ...
        if creds is not None:
            ...
            try:
                resp = await asyncio.to_thread(  # type: ignore[arg-type]
                    intermediate_sts.assume_role, **kwargs
                )
                creds = resp["Credentials"]  # type: ignore[assignment]
            except Exception as exc:
                raise wrap_aws_error(exc, f"Failed to assume role {arn!r}") from exc
        else:
            try:
                resp = await sts.call("AssumeRole", **kwargs)  # type: ignore[assignment]
                creds = resp["Credentials"]  # type: ignore[assignment]
            except Exception as exc:
                raise wrap_aws_error(exc, f"Failed to assume role {arn!r}") from exc

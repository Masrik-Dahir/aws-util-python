"""Native async iot_pipelines — IoT Pipeline utilities.

Native async implementation using :mod:`aws_util.aio._engine` for true
non-blocking I/O.  All Pydantic models are re-exported from the sync module;
only the five public functions are reimplemented here.

See :mod:`aws_util.iot_pipelines` for full docstrings.
"""
...
async def iot_telemetry_to_timestream(
    rule_name: str,
    topic_filter: str,
    database_name: str,
    table_name: str,
    ...
    Returns:
        An :class:`IoTTimestreamRuleResult` with rule ARN, database/table
        names, and creation flags.

    Raises:
        RuntimeError: If any AWS service call fails.
    """
    iot = async_client("iot", region_name)
    ts_write = async_client("timestream-write", region_name)

    # --- Ensure Timestream database exists ---
    created_database = False
    try:
        await ts_write.call("CreateDatabase", DatabaseName=database_name)
        created_database = True
        logger.info("Created Timestream database %s", database_name)
    except RuntimeError as exc:
        exc_str = str(exc).lower()
        if "conflict" not in exc_str and "already" not in exc_str and "exists" not in exc_str:
    ...
    # --- Build and upsert IoT topic rule ---
    ...
    try:
        ...
        exc_str = str(exc).lower()
        if "already" in exc_str or "conflict" in exc_str or "exists" in exc_str:
            try:
                await iot.call(
                    "ReplaceTopicRule",
                    ruleName=rule_name,
    ...
    # --- Retrieve rule ARN ---
    try:
        rule_resp = await iot.call("GetTopicRule", ruleName=rule_name)
...
async def iot_device_shadow_sync(
    thing_name: str,
    table_name: str,
    ...
    Retrieves the classic or named shadow for *thing_name*, computes the
    delta between ``desired`` and ``reported`` states, then persists the
    reconciled record to DynamoDB for fleet management dashboards.

    Args:
        thing_name: IoT thing name whose shadow to read.
        table_name: DynamoDB table name for storing the sync record.
        shadow_name: Named shadow identifier; ``None`` targets the classic shadow.
        region_name: AWS region override.

    Returns:
        A :class:`DeviceShadowSyncResult` with states, delta keys, and
        DynamoDB sync status.

    Raises:
        RuntimeError: If shadow retrieval or DynamoDB write fails.
    """
    iot_data = async_client("iot-data", region_name)
    ...
    try:
        shadow_resp = await iot_data.call("GetThingShadow", **get_kwargs)
        payload_raw = shadow_resp.get("payload", b"{}")
        if isinstance(payload_raw, (bytes, bytearray)):
            shadow_payload: dict[str, Any] = json.loads(payload_raw)
        elif hasattr(payload_raw, "read"):
            shadow_payload = json.loads(payload_raw.read())
        else:
            shadow_payload = payload_raw if isinstance(payload_raw, dict) else {}
    except RuntimeError as exc:
...
async def iot_fleet_command_broadcaster(
    job_id: str,
    thing_group_arn: str,
    ...
    Uploads the JSON *command_document* to S3, creates an IoT Job targeting
    *thing_group_arn*, and stores job metadata in DynamoDB.

    Args:
        job_id: Globally unique IoT Job identifier.
        thing_group_arn: ARN of the thing group to target.
        command_document: Dict representing the job document payload.
        bucket: S3 bucket for the job document.
        table_name: DynamoDB table name for job tracking.
        target_selection: ``"SNAPSHOT"`` or ``"CONTINUOUS"``.
        region_name: AWS region override.

    Returns:
        A :class:`FleetCommandResult` with job ARN, target count, and
        S3 document key.

    Raises:
        RuntimeError: If any AWS service call fails.
        ValueError: If *target_selection* is not valid.
    """
    ...
    # --- Determine targets count via Fleet Indexing (best-effort) ---
    targets_count = 0
    try:
        group_name = thing_group_arn.split(":")[-1].split("/")[-1]
        search_resp = await iot.call(
            "SearchIndex",
...
async def iot_alert_to_sns(
    ...
    Returns:
        An :class:`IoTAlertResult` with current value, breach flag, and
        SNS MessageId when published.
    ...
    sitewise = async_client("iotsitewise", region_name)
    sns = async_client("sns", region_name)

    # --- Fetch latest property value from SiteWise ---
    try:
        prop_resp = await sitewise.call(
...
async def iot_greengrass_component_deployer(
    component_name: str,
    ...
    Reads the ZIP file at *artifact_path* (in a thread to avoid blocking the
    event loop), uploads it to *bucket*, creates a Greengrass v2 component
    version, and triggers a deployment to *target_arn*.

    Args:
        component_name: Fully qualified Greengrass v2 component name.
        component_version: Semantic version string for the component.
        artifact_path: Local filesystem path to the Lambda function ZIP.
        bucket: S3 bucket accessible by Greengrass devices.
        target_arn: ARN of the Greengrass core device or thing group.
        region_name: AWS region override.

    Returns:
        A :class:`GreengrassDeployResult` with component ARN, deployment ID,
        and S3 artifact key.
    ...
    s3 = async_client("s3", region_name)
    gg = async_client("greengrassv2", region_name)

    # --- Read artifact in a thread so the event loop is not blocked ---
    def _read_file() -> bytes:
        return artifact_file.read_bytes()

    artifact_bytes: bytes = await asyncio.get_event_loop().run_in_executor(None, _read_file)

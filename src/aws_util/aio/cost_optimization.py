...
async def trusted_advisor_report_to_s3(
    bucket: str,
    report_key: str,
    ...
    support = async_client("support", region_name or "us-east-1")
    s3 = async_client("s3", region_name)

    try:
        checks_resp = await support.call("DescribeTrustedAdvisorChecks", language="en")
    ...
    for check in all_checks:
        check_id = check["id"]
        try:
            result_resp = await support.call(
                "DescribeTrustedAdvisorCheckResult", checkId=check_id, language="en"
            )
    ...
    try:
        await s3.call("PutObject", Bucket=bucket, Key=report_key, Body=report_body.encode())
    except RuntimeError as exc:
...
async def cost_and_usage_report_analyzer(
    database: str,
    table: str,
    output_location: str,
    ...
    for _ in range(60):
        try:
            status_resp = await athena.call("GetQueryExecution", QueryExecutionId=execution_id)
    ...
    try:
        results_resp = await athena.call("GetQueryResults", QueryExecutionId=execution_id)
...
async def savings_plan_coverage_reporter(
    bucket: str,
    report_key: str,
    start_date: str,
    end_date: str,
    ...
    ce = async_client("ce", region_name)
    s3 = async_client("s3", region_name)
    ...
    try:
        await s3.call("PutObject", Bucket=bucket, Key=report_key, Body=report.encode())
    except RuntimeError as exc:
...
async def ec2_idle_instance_stopper(
    table_name: str,
    ...
    for inst in instances:
        ...
        idle_found += 1
        try:
            ddb_resp = await ddb.call("GetItem", TableName=table_name, Key={"pk": {"S": iid}})
        ...
        if existing:
            ...
            if (now_ts - first_seen) >= grace_seconds:
                try:
                    await ec2.call("StopInstances", InstanceIds=[iid])
                    stopped_count += 1
                except RuntimeError as exc:
                    raise wrap_aws_error(
...
async def rds_idle_snapshot_and_delete(
    lookback_days: int = 7,
    sns_topic_arn: str | None = None,
    dry_run: bool = True,
    region_name: str | None = None,
    ...
    try:
        db_resp = await rds.call("DescribeDBInstances")
    ...
    for inst in instances:
        ...
        try:
            await rds.call(
                "CreateDBSnapshot", DBSnapshotIdentifier=snapshot_id, DBInstanceIdentifier=db_id
            )
            snapshots_created += 1
        except RuntimeError as exc:
            raise wrap_aws_error(
        ...
        if not dry_run:
            try:
                await rds.call(
                    "DeleteDBInstance", DBInstanceIdentifier=db_id, SkipFinalSnapshot=True
                )
                deleted_count += 1
            except RuntimeError as exc:
                raise wrap_aws_error(
...
async def ecr_lifecycle_policy_applier(
    table_name: str,
    ...
    try:
        repos_resp = await ecr.call("DescribeRepositories")
    ...
    for repo in repos:
        ...
        try:
            ...
            err_str = str(exc)
            if "LifecyclePolicyAlreadyExistsException" in err_str:
                already += 1
...
async def s3_intelligent_tiering_enrollor(
    metric_namespace: str,
...
async def lambda_dead_code_detector(
    ...
    for func in functions:
        ...
        if bucket:
            try:
                fn_detail = await lam.call("GetFunction", FunctionName=fn_name)
                ...
                if code_url:
                    ...
                    await s3.call("PutObject", Bucket=bucket, Key=s3_key, Body=code_bytes)
                    archived_count += 1

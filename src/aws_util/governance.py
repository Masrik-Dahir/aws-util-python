"""governance — Cloud governance and compliance utilities.

Provides multi-service governance helpers for AWS environments:

- **config_rule_remediation_executor** — Get non-compliant resources from
  AWS Config, invoke a remediation Lambda for each, record results in
  DynamoDB.
- **organizations_scp_drift_detector** — Compare current SCPs against a
  baseline stored in S3, report drifts via SNS.
- **sso_permission_set_auditor** — Enumerate SSO permission sets and account
  assignments, write compliance report to S3, notify via SNS.
- **kms_key_rotation_auditor** — List KMS customer-managed keys, identify
  those without rotation enabled, write report to S3, alert via SNS.
"""
...
class RemediationResult(BaseModel):
    """Result of the Config rule remediation executor."""
...
class SCPDriftResult(BaseModel):
    """Result of the Organizations SCP drift detector."""

    model_config = ConfigDict(frozen=True)

    total_policies: int
    drifted_policies: int
    drift_details: list[dict[str, Any]] = []


class SSOAuditResult(BaseModel):
    """Result of the SSO permission-set auditor."""

    model_config = ConfigDict(frozen=True)

    permission_sets_count: int
    total_assignments: int
    report_s3_key: str


class KMSRotationAuditResult(BaseModel):
    """Result of the KMS key rotation auditor."""

    model_config = ConfigDict(frozen=True)

    total_keys: int
    rotation_enabled: int
    rotation_disabled: int
    report_s3_key: str
...
def config_rule_remediation_executor(
    ...
    *config_rule_name*, invokes *remediation_lambda_arn* once per resource
    ...
    """
    config = get_client("config", region_name=region_name)
    lambda_client = get_client("lambda", region_name=region_name)
    ddb = get_client("dynamodb", region_name=region_name)

    # ------------------------------------------------------------------
    # Get non-compliant resources for the rule (paginated)
    # ------------------------------------------------------------------
    try:
        ...
        raise wrap_aws_error(
            exc,
            f"config_rule_remediation_executor: failed to get compliance details for {config_rule_name!r}",
        ) from exc
    ...
    for resource in non_compliant:
        ...
        try:
            ...
            raise wrap_aws_error(
                exc,
                f"config_rule_remediation_executor: DynamoDB put_item failed for {resource_id!r}",
            ) from exc
...
def organizations_scp_drift_detector(
    bucket: str,
    baseline_key: str,
    sns_topic_arn: str,
    region_name: str | None = None,
    ...
    Args:
        ...
        baseline_key: S3 object key for the baseline JSON (maps policy ID
            or name to its expected document object).
    ...
    Raises:
        RuntimeError: If any AWS API call fails.
    """
    orgs = get_client("organizations", region_name=region_name)
    ...
    try:
        ...
        baseline: dict[str, Any] = json.loads(obj["Body"].read().decode("utf-8"))
    except ClientError as exc:
        raise wrap_aws_error(
            exc,
    ...
    try:
        ...
        raise wrap_aws_error(exc, "organizations_scp_drift_detector: failed to list SCPs") from exc
    ...
    for policy in policies:
        ...
        try:
            detail = orgs.describe_policy(PolicyId=policy_id)
            current_document = detail.get("Policy", {}).get("Content", "")
        except ClientError as exc:
            raise wrap_aws_error(
                exc,
                f"organizations_scp_drift_detector: describe_policy failed for {policy_id!r}",
            ) from exc
...
def sso_permission_set_auditor(
    instance_arn: str,
    bucket: str,
    report_key: str,
    ...
    Raises:
        RuntimeError: If any AWS API call fails.
    """
    sso = get_client("sso-admin", region_name=region_name)
    s3 = get_client("s3", region_name=region_name)
    ...
    try:
        ...
        raise wrap_aws_error(
            exc, "sso_permission_set_auditor: failed to list permission sets"
        ) from exc
    ...
    for ps_arn in permission_set_arns:
        ...
        try:
            detail_resp = sso.describe_permission_set(
            ...
            raise wrap_aws_error(
                exc,
                f"sso_permission_set_auditor: describe_permission_set failed for {ps_arn!r}",
            ) from exc

        # List account assignments (paginated)
        try:
            assign_paginator = sso.get_paginator("list_accounts_for_provisioned_permission_set")
            ...
            raise wrap_aws_error(
                exc,
                f"sso_permission_set_auditor: list_accounts_for_provisioned_permission_set "
    ...
    try:
        ...
        raise wrap_aws_error(exc, "sso_permission_set_auditor: S3 put_object failed") from exc
...
def kms_key_rotation_auditor(
    ...
    Raises:
        RuntimeError: If any AWS API call fails.
    """
    kms = get_client("kms", region_name=region_name)
    s3 = get_client("s3", region_name=region_name)
    ...
    try:
        ...
        raise wrap_aws_error(exc, "kms_key_rotation_auditor: failed to list keys") from exc
    ...
    for key in all_keys:
        ...
        # Describe key to determine management type
        try:
            ...
            raise wrap_aws_error(
                exc,
                f"kms_key_rotation_auditor: describe_key failed for {key_id!r}",
            ) from exc
        ...
        # Skip keys pending deletion or unavailable
        ...
        try:
            ...
            if error_code in ("UnsupportedOperationException", "AccessDeniedException"):
                rotation_enabled = False
            else:
                raise wrap_aws_error(
                    exc,
                    f"kms_key_rotation_auditor: get_key_rotation_status failed for {key_id!r}",
                ) from exc
    ...
    try:
        ...
        raise wrap_aws_error(exc, "kms_key_rotation_auditor: S3 put_object failed") from exc

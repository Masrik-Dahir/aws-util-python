"""aws_util.inspector --- Amazon Inspector utilities.

Provides functions for managing Amazon Inspector v2: enabling/disabling the
service, managing findings, SBOM exports, coverage, and member accounts.
Uses the ``inspector2`` boto3 service name.
"""
...
class ConfigurationResult(BaseModel):
    ...
    ecr_configuration: dict[str, Any] = {}
    ec2_configuration: dict[str, Any] = {}
    extra: dict[str, Any] = {}
...
class ReportStatusResult(BaseModel):
...
def enable(
    ...
    *,
    ...
    Args:
        resource_types: List of resource types (e.g. ``["EC2", "ECR"]``).
        account_ids: Account IDs to enable (defaults to current account).
    ...
    """
    client = get_client("inspector2", region_name)
    kwargs: dict[str, Any] = {
...
def get_configuration(
    *,
    ...
    Returns:
        A :class:`ConfigurationResult` with configuration details.
...
def update_configuration(
    *,
    ...
    try:
        client.update_configuration(**kwargs)
    except ClientError as exc:
...
def list_finding_aggregations(
    ...
    *,
    ...
    Args:
        aggregation_type: Type of aggregation (e.g. ``FINDING_TYPE``).
    ...
    try:
        ...
        while True:
            if token:
                kwargs["nextToken"] = token
            resp = client.list_finding_aggregations(**kwargs)
...
def create_findings_report(
    *,
    ...
    Args:
        ...
        s3_destination: S3 destination dict with ``bucketName`` and ``kmsKeyArn``.
    ...
    if filter_criteria:
        kwargs["filterCriteria"] = filter_criteria
    try:
        resp = client.create_findings_report(**kwargs)
    except ClientError as exc:
...
def get_sbom_export(
    ...
    *,
    ...
    client = get_client("inspector2", region_name)
    try:
        resp = client.get_sbom_export(reportId=report_id)
    except ClientError as exc:
...
def create_sbom_export(
    *,
    ...
    try:
        resp = client.create_sbom_export(**kwargs)
    except ClientError as exc:
...
def get_member(
    ...
    *,
    ...
    client = get_client("inspector2", region_name)
    try:
        resp = client.get_member(accountId=account_id)
    except ClientError as exc:
...
def _parse_account_status(
    a: dict[str, Any],
) -> AccountStatusResult:
    """Parse a raw account status dict."""
    return AccountStatusResult(
        account_id=a.get("accountId", ""),
        state=a.get("state", {}),
        resource_state=a.get("resourceState", {}),
        extra={k: v for k, v in a.items() if k not in {"accountId", "state", "resourceState"}},
    )


def _parse_finding(f: dict[str, Any]) -> FindingResult:
    """Parse a raw Inspector finding dict."""
...
def _parse_coverage(c: dict[str, Any]) -> CoverageResult:
    """Parse a raw coverage dict."""
    return CoverageResult(
        resource_id=c.get("resourceId", ""),
        resource_type=c.get("resourceType", ""),
        account_id=c.get("accountId", ""),
        scan_status=c.get("scanStatus", {}),
        extra={
            k: v
            for k, v in c.items()
            if k
...
def _parse_member(m: dict[str, Any]) -> MemberResult:
    """Parse a raw Inspector member dict."""
    return MemberResult(
        account_id=m.get("accountId", ""),
        relationship_status=m.get("relationshipStatus", ""),
        delegated_admin_account_id=m.get("delegatedAdminAccountId", ""),
...
class AssociateMemberResult(BaseModel):
...
class BatchAssociateCodeSecurityScanConfigurationResult(BaseModel):
    """Result of batch_associate_code_security_scan_configuration."""

    model_config = ConfigDict(frozen=True)

    failed_associations: list[dict[str, Any]] | None = None
    successful_associations: list[dict[str, Any]] | None = None


class BatchDisassociateCodeSecurityScanConfigurationResult(BaseModel):
...
class BatchGetCodeSnippetResult(BaseModel):
    """Result of batch_get_code_snippet."""

    model_config = ConfigDict(frozen=True)

    code_snippet_results: list[dict[str, Any]] | None = None
    errors: list[dict[str, Any]] | None = None


class BatchGetFindingDetailsResult(BaseModel):
...
class BatchGetFreeTrialInfoResult(BaseModel):
    """Result of batch_get_free_trial_info."""

    model_config = ConfigDict(frozen=True)

    accounts: list[dict[str, Any]] | None = None
    failed_accounts: list[dict[str, Any]] | None = None


class BatchGetMemberEc2DeepInspectionStatusResult(BaseModel):
    """Result of batch_get_member_ec2_deep_inspection_status."""

    model_config = ConfigDict(frozen=True)

    account_ids: list[dict[str, Any]] | None = None
    failed_account_ids: list[dict[str, Any]] | None = None


class BatchUpdateMemberEc2DeepInspectionStatusResult(BaseModel):
...
class CancelFindingsReportResult(BaseModel):
...
class CancelSbomExportResult(BaseModel):
...
class CreateCisScanConfigurationResult(BaseModel):
    """Result of create_cis_scan_configuration."""

    model_config = ConfigDict(frozen=True)

    scan_configuration_arn: str | None = None


class CreateCodeSecurityIntegrationResult(BaseModel):
...
class CreateCodeSecurityScanConfigurationResult(BaseModel):
...
class DeleteCisScanConfigurationResult(BaseModel):
...
class DeleteCodeSecurityIntegrationResult(BaseModel):
...
class DeleteCodeSecurityScanConfigurationResult(BaseModel):
...
class DeleteFilterResult(BaseModel):
...
class DescribeOrganizationConfigurationResult(BaseModel):
    ...
    auto_enable: dict[str, Any] | None = None
    max_account_limit_reached: bool | None = None


class DisableDelegatedAdminAccountResult(BaseModel):
...
class DisassociateMemberResult(BaseModel):
...
class EnableDelegatedAdminAccountResult(BaseModel):
...
class GetCisScanReportResult(BaseModel):
...
class GetCisScanResultDetailsResult(BaseModel):
    """Result of get_cis_scan_result_details."""

    model_config = ConfigDict(frozen=True)

    scan_result_details: list[dict[str, Any]] | None = None
    next_token: str | None = None


class GetClustersForImageResult(BaseModel):
...
class GetCodeSecurityIntegrationResult(BaseModel):
    ...
    last_update_on: str | None = None
    tags: dict[str, Any] | None = None
    authorization_url: str | None = None


class GetCodeSecurityScanResult(BaseModel):
...
class GetCodeSecurityScanConfigurationResult(BaseModel):
...
class GetDelegatedAdminAccountResult(BaseModel):
    """Result of get_delegated_admin_account."""

    model_config = ConfigDict(frozen=True)

    delegated_admin: dict[str, Any] | None = None


class GetEc2DeepInspectionConfigurationResult(BaseModel):
    """Result of get_ec2_deep_inspection_configuration."""

    model_config = ConfigDict(frozen=True)

    package_paths: list[str] | None = None
    org_package_paths: list[str] | None = None
    status: str | None = None
    error_message: str | None = None


class GetEncryptionKeyResult(BaseModel):
...
class ListAccountPermissionsResult(BaseModel):
...
class ListCisScanConfigurationsResult(BaseModel):
    """Result of list_cis_scan_configurations."""

    model_config = ConfigDict(frozen=True)

    scan_configurations: list[dict[str, Any]] | None = None
    next_token: str | None = None


class ListCisScanResultsAggregatedByChecksResult(BaseModel):
    """Result of list_cis_scan_results_aggregated_by_checks."""

    model_config = ConfigDict(frozen=True)

    check_aggregations: list[dict[str, Any]] | None = None
    next_token: str | None = None


class ListCisScanResultsAggregatedByTargetResourceResult(BaseModel):
    """Result of list_cis_scan_results_aggregated_by_target_resource."""

    model_config = ConfigDict(frozen=True)

    target_resource_aggregations: list[dict[str, Any]] | None = None
    next_token: str | None = None


class ListCisScansResult(BaseModel):
...
class ListCodeSecurityIntegrationsResult(BaseModel):
...
class ListCodeSecurityScanConfigurationAssociationsResult(BaseModel):
...
class ListCodeSecurityScanConfigurationsResult(BaseModel):
...
class ListCoverageStatisticsResult(BaseModel):
...
class ListDelegatedAdminAccountsResult(BaseModel):
    """Result of list_delegated_admin_accounts."""

    model_config = ConfigDict(frozen=True)

    delegated_admin_accounts: list[dict[str, Any]] | None = None
    next_token: str | None = None
...
class ListUsageTotalsResult(BaseModel):
...
class SearchVulnerabilitiesResult(BaseModel):
...
class StartCodeSecurityScanResult(BaseModel):
...
class UpdateCisScanConfigurationResult(BaseModel):
...
class UpdateCodeSecurityIntegrationResult(BaseModel):
...
class UpdateCodeSecurityScanConfigurationResult(BaseModel):
...
class UpdateEc2DeepInspectionConfigurationResult(BaseModel):
...
class UpdateFilterResult(BaseModel):
...
class UpdateOrganizationConfigurationResult(BaseModel):
...
def associate_member(
    ...
    try:
        resp = client.associate_member(**kwargs)
    except ClientError as exc:
...
def batch_associate_code_security_scan_configuration(
    ...
    try:
        resp = client.batch_associate_code_security_scan_configuration(**kwargs)
    except ClientError as exc:
        raise wrap_aws_error(
...
def batch_disassociate_code_security_scan_configuration(
    ...
    try:
        resp = client.batch_disassociate_code_security_scan_configuration(**kwargs)
    except ClientError as exc:
        raise wrap_aws_error(
...
def batch_get_code_snippet(
    ...
    try:
        resp = client.batch_get_code_snippet(**kwargs)
    except ClientError as exc:
...
def batch_get_finding_details(
    ...
    kwargs["findingArns"] = finding_arns
    try:
        resp = client.batch_get_finding_details(**kwargs)
    except ClientError as exc:
...
def batch_get_free_trial_info(
    ...
    kwargs["accountIds"] = account_ids
    try:
        resp = client.batch_get_free_trial_info(**kwargs)
    except ClientError as exc:
...
def batch_get_member_ec2_deep_inspection_status(
    *,
    ...
    if account_ids is not None:
        kwargs["accountIds"] = account_ids
    try:
        resp = client.batch_get_member_ec2_deep_inspection_status(**kwargs)
    except ClientError as exc:
...
def batch_update_member_ec2_deep_inspection_status(
    ...
    kwargs["accountIds"] = account_ids
    try:
        resp = client.batch_update_member_ec2_deep_inspection_status(**kwargs)
    except ClientError as exc:
        raise wrap_aws_error(
...
def cancel_findings_report(
    ...
    try:
        resp = client.cancel_findings_report(**kwargs)
    except ClientError as exc:
...
def cancel_sbom_export(
    ...
    kwargs["reportId"] = report_id
    try:
        resp = client.cancel_sbom_export(**kwargs)
    except ClientError as exc:
...
def create_cis_scan_configuration(
    ...
    *,
    ...
    if tags is not None:
        kwargs["tags"] = tags
    try:
        resp = client.create_cis_scan_configuration(**kwargs)
    except ClientError as exc:
...
def create_code_security_integration(
    ...
    *,
    ...
    if tags is not None:
        kwargs["tags"] = tags
    try:
        resp = client.create_code_security_integration(**kwargs)
    except ClientError as exc:
...
def create_code_security_scan_configuration(
    ...
    *,
    ...
    if tags is not None:
        kwargs["tags"] = tags
    try:
        resp = client.create_code_security_scan_configuration(**kwargs)
    except ClientError as exc:
...
def delete_cis_scan_configuration(
    ...
    try:
        resp = client.delete_cis_scan_configuration(**kwargs)
    except ClientError as exc:
...
def delete_code_security_integration(
    ...
    try:
        resp = client.delete_code_security_integration(**kwargs)
    except ClientError as exc:
...
def delete_code_security_scan_configuration(
    ...
    kwargs["scanConfigurationArn"] = scan_configuration_arn
    try:
        resp = client.delete_code_security_scan_configuration(**kwargs)
    except ClientError as exc:
...
def delete_filter(
    ...
    kwargs["arn"] = arn
    try:
        resp = client.delete_filter(**kwargs)
    except ClientError as exc:
...
def disable_delegated_admin_account(
    ...
    try:
        resp = client.disable_delegated_admin_account(**kwargs)
    except ClientError as exc:
...
def disassociate_member(
    ...
    kwargs["accountId"] = account_id
    try:
        resp = client.disassociate_member(**kwargs)
    except ClientError as exc:
...
def enable_delegated_admin_account(
    ...
    *,
    ...
    if client_token is not None:
        kwargs["clientToken"] = client_token
    try:
        resp = client.enable_delegated_admin_account(**kwargs)
    except ClientError as exc:
...
def get_cis_scan_report(
    ...
    *,
    ...
    try:
        resp = client.get_cis_scan_report(**kwargs)
    except ClientError as exc:
...
def get_cis_scan_result_details(
    ...
    *,
    ...
    if max_results is not None:
        kwargs["maxResults"] = max_results
    try:
        resp = client.get_cis_scan_result_details(**kwargs)
    except ClientError as exc:
...
def get_clusters_for_image(
    ...
    *,
    ...
    if next_token is not None:
        kwargs["nextToken"] = next_token
    try:
        resp = client.get_clusters_for_image(**kwargs)
    except ClientError as exc:
...
def get_code_security_integration(
    ...
    *,
    ...
    if tags is not None:
        kwargs["tags"] = tags
    try:
        resp = client.get_code_security_integration(**kwargs)
    except ClientError as exc:
...
def get_code_security_scan(
    ...
    try:
        resp = client.get_code_security_scan(**kwargs)
    except ClientError as exc:
...
def get_code_security_scan_configuration(
    ...
    kwargs["scanConfigurationArn"] = scan_configuration_arn
    try:
        resp = client.get_code_security_scan_configuration(**kwargs)
    except ClientError as exc:
...
def get_delegated_admin_account(
    ...
    try:
        resp = client.get_delegated_admin_account(**kwargs)
    except ClientError as exc:
...
def get_ec2_deep_inspection_configuration(
    ...
    try:
        resp = client.get_ec2_deep_inspection_configuration(**kwargs)
    except ClientError as exc:
...
def get_encryption_key(
    ...
    try:
        resp = client.get_encryption_key(**kwargs)
    except ClientError as exc:
...
def list_account_permissions(
    *,
    ...
    if next_token is not None:
        kwargs["nextToken"] = next_token
    try:
        resp = client.list_account_permissions(**kwargs)
    except ClientError as exc:
...
def list_cis_scan_configurations(
    *,
    ...
    if max_results is not None:
        kwargs["maxResults"] = max_results
    try:
        resp = client.list_cis_scan_configurations(**kwargs)
    except ClientError as exc:
...
def list_cis_scan_results_aggregated_by_checks(
    ...
    *,
    ...
    if max_results is not None:
        kwargs["maxResults"] = max_results
    try:
        resp = client.list_cis_scan_results_aggregated_by_checks(**kwargs)
    except ClientError as exc:
...
def list_cis_scan_results_aggregated_by_target_resource(
    ...
    *,
    ...
    if max_results is not None:
        kwargs["maxResults"] = max_results
    try:
        resp = client.list_cis_scan_results_aggregated_by_target_resource(**kwargs)
    except ClientError as exc:
        raise wrap_aws_error(
...
def list_cis_scans(
    *,
    ...
    if max_results is not None:
        kwargs["maxResults"] = max_results
    try:
        resp = client.list_cis_scans(**kwargs)
    except ClientError as exc:
...
def list_code_security_integrations(
    *,
    ...
    if max_results is not None:
        kwargs["maxResults"] = max_results
    try:
        resp = client.list_code_security_integrations(**kwargs)
    except ClientError as exc:
...
def list_code_security_scan_configuration_associations(
    ...
    *,
    ...
    if max_results is not None:
        kwargs["maxResults"] = max_results
    try:
        resp = client.list_code_security_scan_configuration_associations(**kwargs)
    except ClientError as exc:
        raise wrap_aws_error(
...
def list_code_security_scan_configurations(
    *,
    ...
    if max_results is not None:
        kwargs["maxResults"] = max_results
    try:
        resp = client.list_code_security_scan_configurations(**kwargs)
    except ClientError as exc:
...
def list_coverage_statistics(
    *,
    ...
    if next_token is not None:
        kwargs["nextToken"] = next_token
    try:
        resp = client.list_coverage_statistics(**kwargs)
    except ClientError as exc:
...
def list_delegated_admin_accounts(
    *,
    ...
    if next_token is not None:
        kwargs["nextToken"] = next_token
    try:
        resp = client.list_delegated_admin_accounts(**kwargs)
    except ClientError as exc:
...
def reset_encryption_key(
    ...
    kwargs["resourceType"] = resource_type
    try:
        client.reset_encryption_key(**kwargs)
    except ClientError as exc:
...
def search_vulnerabilities(
    ...
    *,
    ...
    if next_token is not None:
        kwargs["nextToken"] = next_token
    try:
        resp = client.search_vulnerabilities(**kwargs)
    except ClientError as exc:
...
def send_cis_session_health(
    ...
    try:
        client.send_cis_session_health(**kwargs)
    except ClientError as exc:
...
def send_cis_session_telemetry(
    ...
    try:
        client.send_cis_session_telemetry(**kwargs)
    except ClientError as exc:
...
def start_cis_session(
    ...
    try:
        client.start_cis_session(**kwargs)
    except ClientError as exc:
...
def start_code_security_scan(
    ...
    *,
    ...
    if client_token is not None:
        kwargs["clientToken"] = client_token
    try:
        resp = client.start_code_security_scan(**kwargs)
    except ClientError as exc:
...
def stop_cis_session(
    ...
    kwargs["message"] = message
    try:
        client.stop_cis_session(**kwargs)
    except ClientError as exc:
...
def update_cis_scan_configuration(
    ...
    *,
    ...
    if targets is not None:
        kwargs["targets"] = targets
    try:
        resp = client.update_cis_scan_configuration(**kwargs)
    except ClientError as exc:
...
def update_code_security_integration(
    ...
    kwargs["details"] = details
    try:
        resp = client.update_code_security_integration(**kwargs)
    except ClientError as exc:
...
def update_code_security_scan_configuration(
    ...
    kwargs["configuration"] = configuration
    try:
        resp = client.update_code_security_scan_configuration(**kwargs)
    except ClientError as exc:
...
def update_ec2_deep_inspection_configuration(
    *,
    ...
    try:
        resp = client.update_ec2_deep_inspection_configuration(**kwargs)
    except ClientError as exc:
...
def update_encryption_key(
    ...
    kwargs["resourceType"] = resource_type
    try:
        client.update_encryption_key(**kwargs)
    except ClientError as exc:
...
def update_filter(
    ...
    *,
    ...
    if reason is not None:
        kwargs["reason"] = reason
    try:
        resp = client.update_filter(**kwargs)
    except ClientError as exc:
...
def update_org_ec2_deep_inspection_configuration(
    ...
    try:
        client.update_org_ec2_deep_inspection_configuration(**kwargs)
    except ClientError as exc:
...
def update_organization_configuration(
    ...
    try:
        resp = client.update_organization_configuration(**kwargs)
    except ClientError as exc:

...
class LockResult(BaseModel):
    """Result of a distributed lock acquire or release operation."""
...
def distributed_lock_manager(
    ...
    """Acquire or release distributed locks via DynamoDB conditional writes with TTL.

    Uses DynamoDB conditional expressions to implement an atomic distributed
    lock.  Contention events are optionally emitted as CloudWatch metrics.

    - **acquire**: ``put_item`` with ``attribute_not_exists(lock_key) OR expires_at < now``.
    - **release**: ``delete_item`` with ``owner_id = :owner`` condition.

    Args:
        table_name: DynamoDB table name for lock records.
        lock_key: Unique identifier for the lock (primary key).
        owner_id: Identifier for the lock holder (e.g. Lambda request ID).
        ttl_seconds: Lock TTL in seconds (default 30).
        action: ``"acquire"`` or ``"release"``.
        metric_namespace: Optional CloudWatch namespace for contention metrics.
        region_name: AWS region override.

    Returns:
        A :class:`LockResult` with acquire/release outcome.

    Raises:
        ValueError: If *action* is not ``"acquire"`` or ``"release"``.
        RuntimeError: If DynamoDB or CloudWatch API calls fail.
    """
    ...
    if action == "acquire":
        try:
            ...
            logger.info("Lock acquired: %s by %s", lock_key, owner_id)
            return LockResult(
                acquired=True,
                lock_key=lock_key,
                owner_id=owner_id,
            ...
            if error_code == "ConditionalCheckFailedException":
                logger.warning("Lock contention: %s is already held", lock_key)
                if metric_namespace:
                    ...
                    try:
                        cw.put_metric_data(
                            ...
                            MetricData=[
                                {
                                    "MetricName": "LockContention",
                                    "Dimensions": [{"Name": "LockKey", "Value": lock_key}],
                                    "Value": 1.0,
                                    "Unit": "Count",
                                }
                            ],
                        ...
                        logger.warning("Failed to emit contention metric: %s", cw_exc)
                return LockResult(
                    acquired=False,
                    lock_key=lock_key,
                    owner_id=owner_id,
                )
            raise wrap_aws_error(exc, f"Failed to acquire lock {lock_key}") from exc
    ...
    try:
        ...
        logger.info("Lock released: %s by %s", lock_key, owner_id)
        return LockResult(
            acquired=False,
            lock_key=lock_key,
            owner_id=owner_id,
        ...
        error_code = exc.response.get("Error", {}).get("Code", "")
        if error_code == "ConditionalCheckFailedException":
            logger.warning("Release failed: %s not owned by %s", lock_key, owner_id)
            return LockResult(
                acquired=False,
                lock_key=lock_key,
                owner_id=owner_id,
            )
        raise wrap_aws_error(exc, f"Failed to release lock {lock_key}") from exc

"""aws_util.efs — Amazon Elastic File System utilities.

Create, describe, update, and delete EFS file systems, mount targets,
access points, lifecycle configurations, and file-system policies.
"""
...
class FileSystemResult(BaseModel):
    ...
    file_system_id: str
    file_system_arn: str | None = None
    creation_time: str | None = None
    life_cycle_state: str
    performance_mode: str = "generalPurpose"
    throughput_mode: str | None = None
    encrypted: bool = False
    size_in_bytes: dict[str, Any] | None = None
    tags: dict[str, str] = {}
    extra: dict[str, Any] = {}


class MountTargetResult(BaseModel):
...
class AccessPointResult(BaseModel):
...
def _parse_file_system(fs: dict[str, Any]) -> FileSystemResult:
    """Convert a raw EFS file-system dict to a :class:`FileSystemResult`."""
...
def _parse_mount_target(mt: dict[str, Any]) -> MountTargetResult:
    """Convert a raw mount-target dict to a :class:`MountTargetResult`."""
    return MountTargetResult(
        mount_target_id=mt["MountTargetId"],
        file_system_id=mt["FileSystemId"],
        subnet_id=mt["SubnetId"],
        life_cycle_state=mt["LifeCycleState"],
...
def _parse_access_point(ap: dict[str, Any]) -> AccessPointResult:
    """Convert a raw access-point dict to an :class:`AccessPointResult`."""
    tags = ap.get("Tags", [])
    name: str | None = None
    for tag in tags:
        if tag["Key"] == "Name":
...
def create_file_system(
    *,
    ...
    """
    client = get_client("efs", region_name)
...
def create_mount_target(
    ...
    *,
    ...
    try:
        resp = client.create_mount_target(**kwargs)
    except ClientError as exc:
...
def describe_mount_targets(
    *,
    ...
    try:
        while True:
            resp = client.describe_mount_targets(**kwargs)
...
def create_access_point(
    ...
    *,
    ...
    if tags:
        kwargs["Tags"] = [{"Key": k, "Value": v} for k, v in tags.items()]
    try:
        resp = client.create_access_point(**kwargs)
    except ClientError as exc:
...
def describe_access_points(
    *,
    ...
    try:
        while True:
            resp = client.describe_access_points(**kwargs)
...
def describe_file_system_policy(
    ...
    *,
    ...
    try:
        ...
        if code in ("PolicyNotFound", "FileSystemNotFound"):
            return None
...
def delete_file_system_policy(
    ...
    *,
    ...
    client = get_client("efs", region_name)
    try:
        client.delete_file_system_policy(FileSystemId=file_system_id)
    except ClientError as exc:
...
def wait_for_file_system(
    ...
    *,
    ...
    while True:
        results = describe_file_systems(file_system_id=file_system_id, region_name=region_name)
        if not results:
...
class CreateReplicationConfigurationResult(BaseModel):
    """Result of create_replication_configuration."""

    model_config = ConfigDict(frozen=True)

    source_file_system_id: str | None = None
    source_file_system_region: str | None = None
    source_file_system_arn: str | None = None
    original_source_file_system_arn: str | None = None
    creation_time: str | None = None
    destinations: list[dict[str, Any]] | None = None
    source_file_system_owner_id: str | None = None


class DescribeAccountPreferencesResult(BaseModel):
    """Result of describe_account_preferences."""

    model_config = ConfigDict(frozen=True)

    resource_id_preference: dict[str, Any] | None = None
    next_token: str | None = None


class DescribeBackupPolicyResult(BaseModel):
...
class DescribeReplicationConfigurationsResult(BaseModel):
...
class PutAccountPreferencesResult(BaseModel):
...
class PutBackupPolicyResult(BaseModel):
...
class UpdateFileSystemProtectionResult(BaseModel):
...
def create_replication_configuration(
    ...
    try:
        resp = client.create_replication_configuration(**kwargs)
    except ClientError as exc:
...
def delete_replication_configuration(
    ...
    *,
    ...
    try:
        client.delete_replication_configuration(**kwargs)
    except ClientError as exc:
...
def describe_account_preferences(
    *,
    ...
    try:
        resp = client.describe_account_preferences(**kwargs)
    except ClientError as exc:
...
def describe_backup_policy(
    ...
    kwargs["FileSystemId"] = file_system_id
    try:
        resp = client.describe_backup_policy(**kwargs)
    except ClientError as exc:
...
def describe_replication_configurations(
    *,
    ...
    if max_results is not None:
        kwargs["MaxResults"] = max_results
    try:
        resp = client.describe_replication_configurations(**kwargs)
    except ClientError as exc:
...
def put_account_preferences(
    ...
    try:
        resp = client.put_account_preferences(**kwargs)
    except ClientError as exc:
...
def put_backup_policy(
    ...
    try:
        resp = client.put_backup_policy(**kwargs)
    except ClientError as exc:
...
def update_file_system_protection(
    ...
    *,
    ...
    try:
        resp = client.update_file_system_protection(**kwargs)
    except ClientError as exc:

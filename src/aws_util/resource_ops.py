...
# ---------------------------------------------------------------------------
# 11. SES suppression list manager
# ---------------------------------------------------------------------------


class SuppressionSyncResult(BaseModel):
    """Result of :func:`ses_suppression_list_manager`."""
...
def ses_suppression_list_manager(
    bucket: str,
    blocklist_key: str,
    table_name: str,
    region_name: str | None = None,
) -> SuppressionSyncResult:
    """Sync the SES account suppression list with a canonical S3 blocklist.
    ...
    stale entries, and records all operations in DynamoDB for audit.
    ...
    Args:
        ...
        table_name: DynamoDB table for deduplication and audit records.
            Items use ``pk`` (email address) and ``status`` attribute.
        region_name: AWS region override.
    ...
    s3_synced = False
    try:
        obj = s3.get_object(Bucket=bucket, Key=blocklist_key)
    ...
    # Add new suppressed destinations
    added = 0
    for email in to_add:
        try:
    ...
    # Remove stale suppressed destinations
    removed = 0

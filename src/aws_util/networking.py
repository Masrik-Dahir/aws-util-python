...
# ---------------------------------------------------------------------------
# Models — new networking additions
# ---------------------------------------------------------------------------


class LatticeRegistrationResult(BaseModel):
    """Result of :func:`vpc_lattice_service_registrar`."""
...
class TGWAuditResult(BaseModel):
    """Result of :func:`transit_gateway_route_auditor`."""

    model_config = ConfigDict(frozen=True)

    route_tables_checked: int
    total_routes: int
    overlapping_cidrs: list[dict[str, Any]]
    report_s3_key: str


class HealthCheckManagerResult(BaseModel):
    """Result of :func:`route53_health_check_manager`."""
...
class CrossAccountForwardResult(BaseModel):
    """Result of :func:`eventbridge_cross_account_forwarder`."""
...
# ---------------------------------------------------------------------------
# 2. vpc_lattice_service_registrar
# ---------------------------------------------------------------------------


def vpc_lattice_service_registrar(
    ...
    """Register an ECS service or Lambda as a VPC Lattice target.

    Creates a VPC Lattice target group, registers the target, associates it
    with the service network, and creates a Route 53 alias record.

    Args:
        service_network_id: VPC Lattice service network ID or ARN.
        target_type: Target type — ``"INSTANCE"``, ``"IP"``, ``"LAMBDA"``,
            or ``"ALB"``.
        target_id: ARN or ID of the target (Lambda ARN, instance ID, etc.).
        port: Port for the target group.
        hosted_zone_id: Route 53 hosted zone ID for the alias record.
        domain_name: Domain name to create the Route 53 alias under.
        health_check_path: HTTP path for health checks (default ``"/health"``).
        region_name: AWS region override.

    Returns:
        A :class:`LatticeRegistrationResult` with the target group ARN,
        optional service ARN, and Route 53 change ID.

    Raises:
        RuntimeError: If any AWS API call fails.
    """
    lattice = get_client("vpc-lattice", region_name)
    r53 = get_client("route53", region_name)

    # Create target group
    try:
        tg_resp = lattice.create_target_group(
    ...
    # Register target
    target_entry: dict[str, Any] = {"id": target_id, "port": port}
    try:
        lattice.register_targets(targetGroupIdentifier=tg_arn, targets=[target_entry])
    except ClientError as exc:
        raise wrap_aws_error(exc, "vpc_lattice_service_registrar register_targets failed") from exc

    # Associate target group with service network
    service_arn: str | None = None
    try:
        svc_resp = lattice.create_service(
            name=f"svc-{domain_name.replace('.', '-')[:40]}",
        )
        ...
        pass  # service/association creation is best-effort

    # Create Route 53 alias record
    try:
        r53_resp = r53.change_resource_record_sets(
            HostedZoneId=hosted_zone_id,
            ChangeBatch={
                "Changes": [
                    {
...
# ---------------------------------------------------------------------------
# 3. transit_gateway_route_auditor
# ---------------------------------------------------------------------------
...
def transit_gateway_route_auditor(
    transit_gateway_id: str,
    bucket: str,
    report_key: str,
    region_name: str | None = None,
) -> TGWAuditResult:
    """Audit Transit Gateway route tables for overlapping CIDR blocks.

    Enumerates all route tables for the given Transit Gateway, collects
    every active route, detects overlapping CIDR blocks across tables,
    and publishes a JSON compliance report to S3.
    ...
    Returns:
        A :class:`TGWAuditResult` with route table counts and overlaps.
    ...
    try:
        s3.put_object(Bucket=bucket, Key=report_key, Body=json.dumps(report, default=str).encode())
    except ClientError as exc:
...
# ---------------------------------------------------------------------------
# 4. route53_health_check_manager
# ---------------------------------------------------------------------------


def route53_health_check_manager(
    endpoints: list[dict[str, Any]],
    alarm_prefix: str,
    sns_topic_arn: str,
    region_name: str | None = None,
) -> HealthCheckManagerResult:
    """Create Route 53 health checks and CloudWatch alarms for endpoints.

    For each endpoint dict (keys: ``fqdn``, ``port``, ``path``), creates a
    Route 53 health check, then a CloudWatch alarm on ``HealthCheckStatus``
    ...
    Returns:
        A :class:`HealthCheckManagerResult` with counts and IDs.
...
# ---------------------------------------------------------------------------
# 5. eventbridge_cross_account_forwarder
# ---------------------------------------------------------------------------


def eventbridge_cross_account_forwarder(
    rule_name: str,
    ...
    """Create an EventBridge rule to forward events to a target account bus.

    Looks up the source account via STS if not provided, creates an
    EventBridge rule with the given event pattern, and adds the target
    account event bus as the forwarding target.
    ...
    Returns:
        A :class:`CrossAccountForwardResult` with rule ARN, target ID, and
        source account.

    Raises:
        RuntimeError: If any AWS API call fails.
    """
    events_client = get_client("events", region_name)

...
import aws_util.resource_ops as _resource_mod
from aws_util.resource_ops import ses_suppression_list_manager, SuppressionSyncResult
...
class TestSesSuppressionListManager:
    ...
    def test_success_add_and_remove(self, monkeypatch):
        sesv2 = MagicMock()
        ...
        paginator_mock.paginate.return_value = pages
        sesv2.get_paginator.return_value = paginator_mock

        s3 = MagicMock()
        body_mock = MagicMock()
        body_mock.read.return_value = b"keep@example.com\nnew@example.com\n"
        s3.get_object.return_value = {"Body": body_mock}

        dynamo = MagicMock()

        monkeypatch.setattr(
            _resource_mod, "get_client", self._factory(sesv2, s3, dynamo)
        )
        result = ses_suppression_list_manager(
            bucket="my-bucket",
            blocklist_key="blocklist.txt",
            table_name="audit-table",
        )
        assert isinstance(result, SuppressionSyncResult)
        assert result.current_count == 2
        assert result.added == 1  # new@example.com
        assert result.removed == 1  # stale@example.com
        assert result.s3_synced is True
        sesv2.put_suppressed_destination.assert_called_once()
        sesv2.delete_suppressed_destination.assert_called_once()
        assert dynamo.put_item.call_count == 2

    def test_json_blocklist_format(self, monkeypatch):
        sesv2 = MagicMock()
        paginator_mock = MagicMock()
        ...
        s3 = MagicMock()
        body_mock = MagicMock()
        body_mock.read.return_value = b'["a@b.com", "c@d.com"]'
        s3.get_object.return_value = {"Body": body_mock}
        ...
        result = ses_suppression_list_manager("b", "k", "t")
        assert result.added == 2
        assert result.s3_synced is True

    def test_list_suppressed_error(self, monkeypatch):
        sesv2 = MagicMock()
        paginator_mock = MagicMock()
        paginator_mock.paginate.side_effect = _client_error("AccessDenied")
        sesv2.get_paginator.return_value = paginator_mock
        ...
        with pytest.raises(RuntimeError, match="list_suppressed_destinations failed"):
            ses_suppression_list_manager("b", "k", "t")
    ...
    def test_add_suppression_error(self, monkeypatch):
        sesv2 = MagicMock()
        paginator_mock = MagicMock()
        paginator_mock.paginate.return_value = [
            {"SuppressedDestinationSummaries": []}
        ...
        sesv2.get_paginator.return_value = paginator_mock
        sesv2.put_suppressed_destination.side_effect = _client_error(
            "TooManyRequestsException"
        )

        s3 = MagicMock()
        body_mock = MagicMock()
        body_mock.read.return_value = b"bad@example.com\n"
        s3.get_object.return_value = {"Body": body_mock}
    ...
    def test_remove_suppression_error(self, monkeypatch):
        sesv2 = MagicMock()
        paginator_mock = MagicMock()
        paginator_mock.paginate.return_value = [
            {
        ...
        sesv2.get_paginator.return_value = paginator_mock
        sesv2.delete_suppressed_destination.side_effect = _client_error(
            "NotFoundException"
        )
    ...
    def test_no_diff(self, monkeypatch):
        sesv2 = MagicMock()
        paginator_mock = MagicMock()
        paginator_mock.paginate.return_value = [
            {
        ...
        s3 = MagicMock()
        body_mock = MagicMock()
        body_mock.read.return_value = b"user@example.com"
        s3.get_object.return_value = {"Body": body_mock}
        ...
        sesv2.put_suppressed_destination.assert_not_called()
        sesv2.delete_suppressed_destination.assert_not_called()

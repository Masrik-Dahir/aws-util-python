...
class TestCognitoGroupSyncToDynamodb:
    def test_success(self, monkeypatch) -> None:
        ...
        mock_cognito.list_groups.return_value = {
            "Groups": [{"GroupName": "admin"}, {"GroupName": "users"}],
        }
        ...
        def factory(svc, region_name=None):
            return {"cognito-idp": mock_cognito, "dynamodb": mock_ddb}[svc]

        monkeypatch.setattr(mod, "get_client", factory)

        result = cognito_group_sync_to_dynamodb(
            user_pool_id="pool-1", table_name="memberships",
        )
        assert result.groups_synced == 2
        assert result.users_synced == 2  # bob only counted once
        assert result.items_written == 3

    def test_list_groups_error(self, monkeypatch) -> None:
        import aws_util.security_ops as mod

        mock_cognito = MagicMock()
        mock_cognito.list_groups.side_effect = _client_error("ResourceNotFoundException")
        mock_ddb = MagicMock()
        ...
        with pytest.raises(RuntimeError, match="list_groups failed"):
            cognito_group_sync_to_dynamodb(user_pool_id="p", table_name="t")

    def test_list_users_error(self, monkeypatch) -> None:
        import aws_util.security_ops as mod
        ...
        mock_cognito.list_users_in_group.side_effect = _client_error("InternalErrorException")
        mock_ddb = MagicMock()
        ...
        with pytest.raises(RuntimeError, match="list_users_in_group failed"):
            cognito_group_sync_to_dynamodb(user_pool_id="p", table_name="t")

    def test_batch_write_error(self, monkeypatch) -> None:
        import aws_util.security_ops as mod
        ...
        mock_ddb = MagicMock()
        mock_ddb.batch_write_item.side_effect = _client_error("ResourceNotFoundException")
    ...
    def test_empty_groups(self, monkeypatch) -> None:
        import aws_util.security_ops as mod
        ...
        result = cognito_group_sync_to_dynamodb(user_pool_id="p", table_name="t")
...
class TestCloudfrontSignedUrlFactory:
    def test_success(self, monkeypatch) -> None:
        ...
        mock_sm = MagicMock()
        mock_sm.get_secret_value.return_value = {"SecretString": pem}

        def factory(svc, region_name=None):
            return {"secretsmanager": mock_sm}[svc]
        ...
        assert "d123.cloudfront.net" in result.signed_url
        assert "Signature=" in result.signed_url
        assert result.key_pair_id == "APKAEIBAERJR2EXAMPLE"
        assert result.expires_at > 0

    def test_secret_retrieval_error(self, monkeypatch) -> None:
        import aws_util.security_ops as mod
        ...
        with pytest.raises(RuntimeError, match="get_secret failed"):
            cloudfront_signed_url_factory(
                distribution_domain="d", object_path="/p",
                key_pair_id="k", secret_name="s",
            )

    def test_signing_error(self, monkeypatch) -> None:
        import aws_util.security_ops as mod

        mock_sm = MagicMock()
        mock_sm.get_secret_value.return_value = {"SecretString": "not-a-pem"}
...
class TestWafIpBlocklistUpdater:
    def test_success(self, monkeypatch) -> None:
        ...
        mock_s3 = MagicMock()
        body = MagicMock()
        body.read.return_value = b"10.0.0.0/8\n192.168.1.0/24\n"
        mock_s3.get_object.return_value = {"Body": body}

        mock_wafv2 = MagicMock()
        mock_wafv2.get_ip_set.return_value = {
            "IPSet": {"Addresses": ["10.0.0.0/8"]},
            "LockToken": "token-1",
        }
        mock_wafv2.update_ip_set.return_value = {}

        mock_ddb = MagicMock()
        mock_ddb.put_item.return_value = {}

        def factory(svc, region_name=None):
            return {"s3": mock_s3, "wafv2": mock_wafv2, "dynamodb": mock_ddb}[svc]
        ...
        result = waf_ip_blocklist_updater(
            ...
            bucket="feeds",
            feed_key="ips.txt",
            table_name="waf_history",
        )
        assert result.ips_added == 1  # 192.168.1.0/24 is new
        assert result.ips_removed == 0  # 10.0.0.0/8 is still in feed
    ...
    def test_s3_read_error(self, monkeypatch) -> None:
        ...
        with pytest.raises(RuntimeError, match="s3 get_object failed"):
            waf_ip_blocklist_updater(
                ip_set_name="n", ip_set_id="i", scope="REGIONAL",
                bucket="b", feed_key="k", table_name="t",
            )

    def test_get_ip_set_error(self, monkeypatch) -> None:
        import aws_util.security_ops as mod
        ...
        mock_wafv2 = MagicMock()
        mock_wafv2.get_ip_set.side_effect = _client_error("WAFNonexistentItemException")
        mock_ddb = MagicMock()
        ...
        with pytest.raises(RuntimeError, match="get_ip_set failed"):
            waf_ip_blocklist_updater(
                ip_set_name="n", ip_set_id="i", scope="REGIONAL",
                bucket="b", feed_key="k", table_name="t",
            )

    def test_update_ip_set_error(self, monkeypatch) -> None:
        import aws_util.security_ops as mod
        ...
        mock_wafv2 = MagicMock()
        mock_wafv2.get_ip_set.return_value = {
            "IPSet": {"Addresses": []}, "LockToken": "tok",
        }
        mock_wafv2.update_ip_set.side_effect = _client_error("WAFOptimisticLockException")
        mock_ddb = MagicMock()
        ...
        with pytest.raises(RuntimeError, match="update_ip_set failed"):
            waf_ip_blocklist_updater(
                ip_set_name="n", ip_set_id="i", scope="REGIONAL",
                bucket="b", feed_key="k", table_name="t",
            )
    ...
    def test_ddb_put_error(self, monkeypatch) -> None:
        ...
        with pytest.raises(RuntimeError, match="ddb put_item failed"):
            waf_ip_blocklist_updater(
                ip_set_name="n", ip_set_id="i", scope="REGIONAL",
                bucket="b", feed_key="k", table_name="t",
            )
...
class TestShieldAdvancedProtectionManager:
    def test_success_new_protections(self, monkeypatch) -> None:
        import aws_util.security_ops as mod

        mock_shield = MagicMock()
        mock_shield.create_protection.return_value = {}
        mock_shield.update_emergency_contact_settings.return_value = {}

        def factory(svc, region_name=None):
            return {"shield": mock_shield}[svc]

        monkeypatch.setattr(mod, "get_client", factory)

        result = shield_advanced_protection_manager(
            resource_arns=["arn:aws:elasticloadbalancing:us-east-1:123:lb"],
            emergency_contact_email="ops@example.com",
        )
        assert result.protections_created == 1
        assert result.already_protected == 0
        assert result.emergency_contact_set is True

    def test_already_exists(self, monkeypatch) -> None:
        import aws_util.security_ops as mod

        mock_shield = MagicMock()
        mock_shield.create_protection.side_effect = _client_error("ResourceAlreadyExistsException")
        ...
        result = shield_advanced_protection_manager(
            resource_arns=["arn:aws:ec2:us-east-1:123:eip"],
        )
        assert result.protections_created == 0
        assert result.already_protected == 1
        assert result.emergency_contact_set is False

    def test_create_protection_other_error(self, monkeypatch) -> None:
        import aws_util.security_ops as mod

        mock_shield = MagicMock()
        mock_shield.create_protection.side_effect = _client_error("InvalidParameterException")
        ...
        with pytest.raises(RuntimeError, match="shield_advanced_protection_manager failed"):
            shield_advanced_protection_manager(
                resource_arns=["arn:aws:ec2:us-east-1:123:eip"],
            )

    def test_emergency_contact_error(self, monkeypatch) -> None:
        import aws_util.security_ops as mod

        mock_shield = MagicMock()
        mock_shield.create_protection.return_value = {}
        mock_shield.update_emergency_contact_settings.side_effect = _client_error("InternalErrorException")
        ...
        with pytest.raises(RuntimeError, match="emergency contact failed"):
            shield_advanced_protection_manager(
                resource_arns=["arn:aws:ec2:us-east-1:123:eip"],
                emergency_contact_email="ops@example.com",
            )

    def test_no_emergency_contact(self, monkeypatch) -> None:
        import aws_util.security_ops as mod
        ...
        assert result.emergency_contact_set is False
        mock_shield.update_emergency_contact_settings.assert_not_called()
...
class TestAcmCertificateExpiryMonitor:
    def test_expiring_certs(self, monkeypatch) -> None:
        import aws_util.security_ops as mod
        ...
        # cert1 expires soon, cert2 expires later
        now = datetime.now(tz=timezone.utc)
        mock_acm.describe_certificate.side_effect = [
            {"Certificate": {"DomainName": "example.com", "NotAfter": now + timedelta(days=10)}},
            {"Certificate": {"DomainName": "safe.com", "NotAfter": now + timedelta(days=90)}},
        ]
        mock_sns = MagicMock()
        mock_sns.publish.return_value = {}

        def factory(svc, region_name=None):
            return {"acm": mock_acm, "sns": mock_sns}[svc]
        ...
        assert "example.com" in result.expiring_domains
        assert result.alert_sent is True

    def test_no_expiring_certs(self, monkeypatch) -> None:
        import aws_util.security_ops as mod
        from datetime import datetime, timezone, timedelta
        ...
        mock_acm.describe_certificate.return_value = {
            "Certificate": {"DomainName": "ok.com", "NotAfter": now + timedelta(days=365)},
        }
        mock_sns = MagicMock()
        ...
        result = acm_certificate_expiry_monitor(
            sns_topic_arn="arn:sns", days_threshold=30,
        )
        assert result.expiring_count == 0
        assert result.alert_sent is False
        mock_sns.publish.assert_not_called()

    def test_list_certificates_error(self, monkeypatch) -> None:
        import aws_util.security_ops as mod

        mock_acm = MagicMock()
        mock_acm.list_certificates.side_effect = _client_error("AccessDeniedException")
        mock_sns = MagicMock()
        ...
        with pytest.raises(RuntimeError, match="list_certificates failed"):
            acm_certificate_expiry_monitor(sns_topic_arn="arn:sns")
    ...
    def test_sns_publish_error(self, monkeypatch) -> None:
        ...
        now = datetime.now(tz=timezone.utc)
        mock_acm.describe_certificate.return_value = {
            "Certificate": {"DomainName": "expiring.com", "NotAfter": now + timedelta(days=5)},
        }
        mock_sns = MagicMock()
        mock_sns.publish.side_effect = _client_error("AuthorizationError")
    ...
    def test_describe_certificate_error_skipped(self, monkeypatch) -> None:
        import aws_util.security_ops as mod
        ...
        mock_acm.describe_certificate.side_effect = _client_error("ResourceNotFoundException")
        mock_sns = MagicMock()
        ...
        result = acm_certificate_expiry_monitor(sns_topic_arn="arn:sns")
        # describe_certificate error is caught and continued
        assert result.expiring_count == 0
...
class TestCognitoPreTokenEnricher:
    def test_success(self, monkeypatch) -> None:
        ...
        result = cognito_pre_token_enricher(
            ...
            claim_attributes=["role", "tier"],
        )
        assert result.username == "alice"
        assert result.claims_added == 2
        assert result.claims["role"] == "admin"
        assert result.claims["tier"] == "premium"
        assert result.claims["email"] == "alice@example.com"

    def test_admin_get_user_error(self, monkeypatch) -> None:
        import aws_util.security_ops as mod
        ...
        with pytest.raises(RuntimeError, match="admin_get_user failed"):
            cognito_pre_token_enricher(
                user_pool_id="p", username="u",
                table_name="t", claim_attributes=["role"],
            )

    def test_ddb_get_error(self, monkeypatch) -> None:
        import aws_util.security_ops as mod

        mock_cognito = MagicMock()
        mock_cognito.admin_get_user.return_value = {"UserAttributes": []}
        mock_ddb = MagicMock()
        mock_ddb.get_item.side_effect = _client_error("ResourceNotFoundException")
        ...
        with pytest.raises(RuntimeError, match="ddb get_item failed"):
            cognito_pre_token_enricher(
                user_pool_id="p", username="u",
                table_name="t", claim_attributes=["role"],
            )

    def test_no_custom_attrs(self, monkeypatch) -> None:
        import aws_util.security_ops as mod
        ...
        mock_ddb = MagicMock()
        mock_ddb.get_item.return_value = {"Item": {"pk": {"S": "u"}}}
        ...
        result = cognito_pre_token_enricher(
            user_pool_id="p", username="u",
            table_name="t", claim_attributes=["missing_attr"],
        )
